...
- 源码事实源中的随包模块：血DK `1.2.1.61`，奶骑 `1.2.1.28`；最近可见本地验收包仍早于当前源码，不能代表当前运行态。
...
- 运行健康协议固定使用状态索引 `502`/`503` 的 `Fuyutsui协议版本`/`Fuyutsui状态心跳` 字段，当前 Fuyutsui 协议版本为 `6`。Fuyutsui 每约 200 ms 更新心跳；生产运行时只有在版本匹配且心跳持续变化时才评估并发送逻辑。字段缺失、版本不匹配或心跳冻结会保持逻辑等待并禁止按键发送。
