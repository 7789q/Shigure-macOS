...
- 源码事实源中的随包模块：血DK `1.2.1.61`，奶骑 `1.2.1.25`；既有本地验收包已按当时源码重新打包，但未包含本轮最新 Runtime 动作确认修复。
- 本地验证状态：Release 构建 0 警告、0 错误；Core 契约测试 89 passed、0 failed；改动 Lua 和 macOS shell 语法检查通过。
- 本地验收包：`artifacts/macos/Shigure-20260908-local-4.app`（arm64，已完成签名校验并通过 Launch Services 启动验收）；未安装到 `/Applications`，WoW 实战验收仍待完成。
...
- 运行健康协议固定使用状态索引 `502`/`503` 的 `Fuyutsui协议版本`/`Fuyutsui状态心跳` 字段，当前协议版本为 `2`。Fuyutsui 每约 200 ms 更新心跳；生产运行时只有在版本匹配且心跳持续变化时才评估并发送逻辑。字段缺失、版本不匹配或心跳冻结会保持逻辑等待并禁止按键发送。
