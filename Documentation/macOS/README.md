...
- 目标类型协议中，敌对当前目标统一编码为 `1`；队伍外友方玩家为 `151`，队伍外友方 NPC 为 `152`。正义盾击等敌对技能不得依赖旧的单位索引编码（如 `51`）判断目标类型。
...
- 运行时先投递一次动作并等待 WoW 的玩家施法 START/SUCCEEDED 回写；若首次投递落在 GCD 内，只在真实 GCD 剩余不高于 50ms 时补发一次。读条期间普通动作保持静默，更高优先级治疗可抢占；冷却、充能、圣能与增益变化仍作为兼容确认来源。
