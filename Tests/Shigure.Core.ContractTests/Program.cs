...
var tests = new (string Name, Action Run)[]
{
    ...
    ("bundled holy paladin raid module replay", () => HolyPaladinRaidContractTests.Run(FindRepositoryRoot())),
    ...
    ("target identity stability gate contract", TargetIdentityStabilityGateContract),
    ...
    ("runtime macro binding gate contract", RuntimeMacroBindingGateContract),
    ("runtime protocol health contract", RuntimeProtocolHealthContract),
    ...
    ("periodic refresh production Lua replay", PeriodicRefreshProductionLuaReplayContract),
...
var skipContractSurfaceManifest = args.Any(filter =>
    string.Equals(filter, "--skip-contract-surface-manifest", StringComparison.Ordinal));
var testFilters = args
    .Where(filter => !string.Equals(filter, "--skip-contract-surface-manifest", StringComparison.Ordinal))
    .ToArray();
var selectedTests = testFilters.Length == 0
    ? tests
    : tests.Where(test => testFilters.Any(filter =>
        test.Name.Contains(filter, StringComparison.OrdinalIgnoreCase))).ToArray();
if (skipContractSurfaceManifest)
{
    selectedTests = selectedTests
        .Where(test => !string.Equals(test.Name, "contract surface manifest", StringComparison.Ordinal))
        .ToArray();
}
...
if (skipContractSurfaceManifest)
{
    Console.WriteLine("SKIP contract surface manifest (local packaging mode)");
}
...
static void BundledBloodDeathbringerModuleContract()
{
    var repositoryRoot = FindRepositoryRoot();
    var modulePath = Path.Combine(repositoryRoot, "BundledModules", "blood-deathbringer-12.1.json");
    ...
    Equal("1.2.1.61", module.Version, "blood deathbringer module version");
    ...
    Equal(49, module.Rules.Count, "blood deathbringer module keeps the full priority matrix");
    ...
    Equal(true, module.Rules.Any(rule => rule.Spell == "死神印记"
        && rule.Condition.Contains("副本内 == false", StringComparison.Ordinal)
        && rule.Condition.Contains("目标类型 > 0", StringComparison.Ordinal)
        && rule.Condition.Contains("目标类型 < 100", StringComparison.Ordinal)
        && !rule.Condition.Contains("目标类型 > 90", StringComparison.Ordinal)),
        "Reaper's Mark accepts any valid target outside an instance");
    Equal(true, module.Rules.Any(rule => rule.Spell == "死神印记"
        && rule.Condition.Contains("副本内 == true", StringComparison.Ordinal)
        && rule.Condition.Contains("目标类型 > 90", StringComparison.Ordinal)),
        "Reaper's Mark keeps the high-value target gate inside an instance");
    ...
    Equal(true, module.Rules.Any(rule => rule.Spell == "血液沸腾"
        && rule.Condition.Contains("血沸手动触发 == true", StringComparison.Ordinal)
        && rule.Condition.Contains("血沸自动链 == false", StringComparison.Ordinal)),
        "Blood Boil only sends the first key for a pending highlighted trigger");
    Equal(true, module.Rules.Where(rule => rule.Spell == "血液沸腾" && rule.Enabled)
            .Where(rule => rule.Condition.Contains("血沸高亮 == false", StringComparison.Ordinal))
            .All(rule => rule.Condition.Contains("血沸手动触发 == false", StringComparison.Ordinal)
                && rule.Condition.Contains("血沸自动链 == false", StringComparison.Ordinal)),
        "normal Blood Boil is blocked while the SeUI-style highlight state is active");
    Equal(3, module.Rules.Count(rule => rule.Spell == "血液沸腾" && rule.Enabled && rule.Condition != "false"),
        "Blood Boil has highlighted, moving-pull and normal independent branches");
    ...
    var highlightedBloodBoilIndex = module.Rules.FindIndex(rule =>
        rule.Spell == "血液沸腾" && rule.Condition.Contains("血沸手动触发 == true", StringComparison.Ordinal));
    ...
    var movingBloodBoilIndex = module.Rules.FindIndex(rule =>
        rule.Spell == "血液沸腾"
        && rule.Comment.Contains("移动拉怪血沸保留", StringComparison.Ordinal));
    ...
    var raiseDeadIndex = module.Rules.FindIndex(rule => rule.Spell == "亡者复生");
    var lichborneIndex = module.Rules.FindIndex(rule => rule.Spell == "巫妖之躯");
    ...
    Equal(true, movingBloodBoilIndex >= 0
        && module.Rules[movingBloodBoilIndex].Condition.Contains("移动 == true", StringComparison.Ordinal)
        && module.Rules[movingBloodBoilIndex].Condition.Contains("spells.血液沸腾层数 >= 2", StringComparison.Ordinal),
        "moving pull uses a guarded two-charge Blood Boil branch");
    ...
    Equal(false, module.Rules[highlightedBloodBoilIndex].Condition.Contains("生命值 <= 60", StringComparison.Ordinal)
        || module.Rules[highlightedBloodBoilIndex].Condition.Contains("符文能量 < 80", StringComparison.Ordinal),
        "highlighted Blood Boil has no health or runic-power gate");
    Equal(false, module.Rules[highlightedBloodBoilIndex].Condition.Contains("目标血之疫病", StringComparison.Ordinal)
        || module.Rules[highlightedBloodBoilIndex].Condition.Contains("血沸循环心打次数", StringComparison.Ordinal),
        "highlighted Blood Boil does not wait for disease or cycle-count state");
    Equal(false, module.Rules.Any(rule => rule.Spell == "血液沸腾"
        && rule.Condition.Contains("spells.血液沸腾 == 0", StringComparison.Ordinal)),
        "Blood Boil rules rely on charge state instead of the stale cooldown field");
    Equal(true, module.Rules.Any(rule => rule.Spell == "心脏打击"
        && rule.Condition.Contains("auras.午夜舞步 > 0", StringComparison.Ordinal)),
        "Midnight Walk proc makes Heart Strike the second highlighted spender");
    Equal(false, module.Rules.Any(rule => rule.Spell == "心脏打击"
        && rule.Condition.Contains("spells.心脏打击 == 0", StringComparison.Ordinal)),
    ...
    Equal(true, module.Rules.Any(rule => rule.Spell == "精髓分裂"
        && rule.Condition.Contains("auras.血债层数 >= 10", StringComparison.Ordinal)),
        "Marrowrend has a Blood Debt threshold branch");
    ...
    Equal(true, module.Rules.Any(rule => rule.Spell == "灵界打击"
        && rule.Condition.Contains("符文能量 >= 80", StringComparison.Ordinal)
    ...
    Equal(true, module.Rules.Any(rule => rule.Spell == "精髓分裂"
        && rule.Condition.Contains("auras.血债层数 >= 10", StringComparison.Ordinal)
        && !rule.Condition.Contains("auras.白骨之盾层数 >= 1", StringComparison.Ordinal)),
        "Blood Debt Marrowrend can recover from an already expired Bone Shield");
    ...
    Equal(true, module.Rules.Any(rule => rule.Spell == "血液沸腾"
            && rule.Enabled
            && rule.Condition.Contains("血沸自动链 == false", StringComparison.Ordinal)
            && rule.Condition.Contains("血沸即将满层 == true", StringComparison.Ordinal)
            && rule.Condition.Contains("auras.目标血之疫病 == 0", StringComparison.Ordinal)
        ...
        "normal Blood Boil repairs disease and predicts the next charge cap without resource gates");
    Equal(false, module.Rules.Any(rule => rule.Spell == "心脏打击"
        && rule.Enabled
    ...
    Equal(true, module.Rules
            .Where(rule => (rule.Spell is "心脏打击")
                && (rule.Condition.Contains("auras.沸点", StringComparison.Ordinal)
                    || rule.Condition.Contains("血沸高亮 == true", StringComparison.Ordinal)
                    || rule.Condition.Contains("auras.午夜舞步", StringComparison.Ordinal)
                    || rule.Condition.Contains("血沸循环心打次数", StringComparison.Ordinal)))
            .All(rule => rule.Condition.Contains("符文能量 < 80", StringComparison.Ordinal)
                || rule.Condition.Contains("符文能量 < 40", StringComparison.Ordinal)),
        "Heart Strike output stays below the Death Strike line; highlighted Blood Boil may preempt it");
    Equal(false, module.Rules.Any(rule => rule.Spell == "精髓分裂"
        && rule.Enabled
    ...
    var shortCombatDeathAndDecayRules = module.Rules.Where(rule => rule.Spell == "枯萎凋零"
        ...
        .ToArray();
    Equal(true, shortCombatDeathAndDecayRules.Length == 1
        && shortCombatDeathAndDecayRules[0].Condition.Contains("移动 == false", StringComparison.Ordinal)
        && shortCombatDeathAndDecayRules[0].Condition.Contains("站定时长 >= 1", StringComparison.Ordinal)
        && shortCombatDeathAndDecayRules[0].Condition.Contains("auras.血色之地 == 0", StringComparison.Ordinal)
        && shortCombatDeathAndDecayRules[0].Condition.Contains("spells.枯萎凋零层数 >= 1", StringComparison.Ordinal)
        && shortCombatDeathAndDecayRules[0].DelayMs == 5000
        && shortCombatDeathAndDecayRules[0].LogicDelayMs == 300,
        "Death and Decay requires one second stationary before fifteen seconds");
    var longCombatDeathAndDecayRules = module.Rules.Where(rule => rule.Spell == "枯萎凋零"
            && rule.Condition.Contains("战斗时间 >= 15", StringComparison.Ordinal))
        .ToArray();
    ...
    Equal(2, module.Rules.Count(rule => rule.Spell == "枯萎凋零"),
        "Death and Decay uses exactly the short and long combat branches");
    Equal(true, module.Rules.Where(rule => rule.Spell == "血液沸腾").All(rule => rule.LogicDelayMs == 300),
        "Blood Boil confirmation does not pause the state scanner for a full GCD");
    var deathKnightClass = File.ReadAllText(Path.Combine(repositoryRoot, "Fuyutsui", "class", "DeathKnight.lua"));
    ...
    Equal(true, module.Rules.Any(rule => rule.Spell == "血液沸腾"
        ...
        && rule.Condition.Contains("spells.血液沸腾层数 >= 1", StringComparison.Ordinal)
        && rule.Condition.Contains("auras.目标血之疫病 == 0", StringComparison.Ordinal)
        && rule.Condition.Contains("血沸高亮 == false", StringComparison.Ordinal)
        && rule.Condition.Contains("血沸手动触发 == false", StringComparison.Ordinal)
        && !rule.Condition.Contains("血沸循环心打次数", StringComparison.Ordinal)
        && !rule.Condition.Contains("符文 == 0", StringComparison.Ordinal)
        && !rule.Condition.Contains("符文能量 < 40", StringComparison.Ordinal)
        && rule.Condition.Contains("血沸自动链 == false", StringComparison.Ordinal)),
        "Blood Boil uses disease repair and predictive charge-cap protection without resource gates");
    ...
    var diagnosticModule = module.Clone();
    foreach (var rule in diagnosticModule.Rules)
    {
        rule.Hotkey = "CTRL-A";
    }
    var diagnosticState = new GameState(new Dictionary<string, object?>
    {
        ["职业"] = 6,
        ["专精"] = 1,
        ["英雄天赋"] = 1,
    ...
    var diagnosticDecision = ModuleLogic.Run(
        diagnosticModule,
        diagnosticState,
        new ContractKeymapResolver());
    Equal("心脏打击", diagnosticDecision.UnitInfo["动作技能"],
        "blood diagnostic replay selects Heart Strike as the no-proc filler");
    Equal(4, Convert.ToInt32(diagnosticDecision.UnitInfo["符文"]),
        "blood diagnostic replay exposes the rune snapshot");
    Equal(70, Convert.ToInt32(diagnosticDecision.UnitInfo["符文能量"]),
        "blood diagnostic replay exposes runic power");
    Equal(1, Convert.ToInt32(diagnosticDecision.UnitInfo["敌人数量"]),
        "blood diagnostic replay exposes enemy count");
    Equal("符文", diagnosticDecision.CooldownConfirmationStateField,
        "blood rune spender confirms through the rune resource field");
    Equal(4, diagnosticDecision.CooldownConfirmationInitialValue,
        "blood rune confirmation records the pre-cast rune count");
    Equal(true, diagnosticDecision.AllowResourceOnlyConfirmation,
        "blood rune confirmation can work without a matching action event");

    var targetInterruptDecision = ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(targetCastRemaining: 5, targetCastInterruptible: 1),
        new ContractKeymapResolver());
    Equal("心灵冰冻", targetInterruptDecision.UnitInfo["动作技能"],
        "target interrupt wins during the final half-second of an interruptible cast");
    Equal(ReservedUnit.Target, Convert.ToInt32(targetInterruptDecision.UnitInfo["动作单位槽位"]),
        "target interrupt resolves the target macro slot");
    Equal(true, targetInterruptDecision.AllowCastPreemption,
    ...
    var focusInterruptDecision = ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(focusCastRemaining: 5, focusCastInterruptible: 1),
        new ContractKeymapResolver());
    Equal("心灵冰冻", focusInterruptDecision.UnitInfo["动作技能"],
        "focus interrupt wins during the final half-second of an interruptible cast");
    Equal(ReservedUnit.Focus, Convert.ToInt32(focusInterruptDecision.UnitInfo["动作单位槽位"]),
        "focus interrupt resolves the focus macro slot");

    var outsideInterruptWindowDecision = ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(targetCastRemaining: 6, targetCastInterruptible: 1),
        new ContractKeymapResolver());
    ...
    static GameState BurstState(
        ...
        int burstEnabled,
        int runicPower = 70,
        int playerDeathAndDecayAura = 0,
        int deathAndDecayCharges = 0,
        bool moving = false,
        int targetType = 92,
        int targetRange = 3,
        int stationaryDuration = 3,
        int raiseDeadCooldown = 1,
        int lichborneCooldown = 1,
        int combatTime = 1,
        ...
        new(new Dictionary<string, object?>
        {
            ...
            ["生命值"] = 100,
            ["移动"] = moving,
            ["站定时长"] = moving ? 0 : stationaryDuration,
            ["符文"] = 1,
            ["符文能量"] = runicPower,
            ["敌人数量"] = enemyCount,
            ["敌人数-有仇恨"] = 1,
            ...
            ["auras"] = new Dictionary<string, object?>
            {
                ...
                ["沸点层数"] = 0,
                ["符文刃舞"] = 0,
                ["枯萎凋零"] = playerDeathAndDecayAura,
                ["血色之地"] = playerDeathAndDecayAura,
                ["赤色天灾"] = 0,
                ["吸血鬼之血"] = 0,
                ["冰封之韧"] = 0
            },
            ...
            {
                ["灵界打击"] = 0,
                ["亡者复生"] = raiseDeadCooldown,
                ["巫妖之躯"] = lichborneCooldown,
                ["符文刃舞"] = 0,
                ["死神印记"] = 0,
                ["枯萎凋零"] = 0,
                ["枯萎凋零层数"] = deathAndDecayCharges,
                ["血液沸腾"] = 0,
                ["血液沸腾层数"] = 0,
                ["精髓分裂"] = 0,
                ["心脏打击"] = 0,
    ...
    GameState BloodRotationState(
        int runes = 2,
        int runicPower = 70,
        int bloodBoilCharges = 0,
        int boilingPoint = 0,
        int midnightWalk = 0,
        int exterminate = 0,
        int targetType = 1,
        ...
        int boneShield = 5,
        bool moving = false,
        int bloodBoilHeartStrikes = 0,
        int heartStrikeCooldown = 0,
        int bloodBoilCooldown = 0,
        int bloodDebt = 0,
        int boneShieldRemaining = 0,
        int targetCastRemaining = 0,
        int targetCastInterruptible = 0,
        int focusCastRemaining = 0,
        int focusCastInterruptible = 0,
        int focusRange = 3,
        bool bloodBoilManualTrigger = false,
        bool bloodBoilAutoChain = false,
        bool inInstance = true,
        int health = 100,
        int bloodBoilChargeRemaining = 200,
        int gcdDuration = 100,
        bool bloodBoilNearCap = false) =>
        new(new Dictionary<string, object?>
        {
            ["战斗时间"] = 1,
            ["生命值"] = health,
            ...
            ["焦点距离"] = focusRange,
            ["血沸手动触发"] = bloodBoilManualTrigger,
            ["血沸自动链"] = bloodBoilAutoChain,
            ["血沸高亮"] = bloodBoilManualTrigger || bloodBoilAutoChain,
            ["副本内"] = inInstance,
            ["符文刃舞爆发窗口"] = 0,
            ["血沸循环心打次数"] = bloodBoilHeartStrikes,
            ["血沸充能剩余"] = bloodBoilChargeRemaining,
            ["血沸即将满层"] = bloodBoilNearCap,
            ["公共冷却时长"] = gcdDuration,
            ["auras"] = new Dictionary<string, object?>
            {
                ["白骨之盾层数"] = boneShield,
                ["白骨之盾"] = boneShieldRemaining,
                ["血债层数"] = bloodDebt,
                ["沸点"] = boilingPoint,
                ["沸点层数"] = 0,
                ["午夜舞步"] = midnightWalk,
            ...
            ["spells"] = new Dictionary<string, object?>
            {
                ...
                ["枯萎凋零"] = 0,
                ["枯萎凋零层数"] = 0,
                ["血液沸腾"] = bloodBoilCooldown,
                ["血液沸腾层数"] = bloodBoilCharges,
                ["精髓分裂"] = 0,
                ["心脏打击"] = heartStrikeCooldown,
                ["心灵冰冻"] = 0,
                ["符文分流"] = 1,
                ["吸血鬼之血"] = 1,
                ["冰封之韧"] = 1,
    ...
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, bloodBoilCharges: 1, boilingPoint: 1, boneShield: 12, bloodBoilManualTrigger: true),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "highlighted Blood Boil outranks other highlighted spenders");
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, bloodBoilCharges: 1, boilingPoint: 1, targetDisease: 1, boneShield: 12, bloodBoilManualTrigger: true),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "highlighted Blood Boil is not delayed by an existing disease aura or cycle count");
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, bloodBoilCharges: 1, boilingPoint: 1, targetDisease: 1, boneShield: 12, bloodBoilHeartStrikes: 2, bloodBoilManualTrigger: true),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "highlighted Blood Boil is independent of the Heart Strike count state");
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, bloodBoilCharges: 1, bloodBoilHeartStrikes: 2, boneShield: 12, bloodBoilManualTrigger: true),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "active SeUI-style highlight state keeps Blood Boil available after repeated Heart Strikes");
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 85, bloodBoilCharges: 1, boilingPoint: 1, boneShield: 12, bloodBoilManualTrigger: true),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "highlighted Blood Boil preempts high-runic-power Death Strike");
    Equal("灵界打击", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 85, boneShield: 0),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "high runic power Death Strike remains available when Bone Shield has expired");
    var lowHealthHighlightedState = BloodRotationState(
        runes: 2,
        runicPower: 70,
        bloodBoilCharges: 1,
        boilingPoint: 1,
        bloodBoilManualTrigger: true);
    ...
    Equal(false, Equals("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 30, bloodBoilCharges: 1, boneShield: 12, bloodBoilAutoChain: true),
        new ContractKeymapResolver()).UnitInfo["动作技能"]),
        "automatic Blood Boil chain never sends another manual Blood Boil key");
    ...
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 70, bloodBoilCharges: 2, targetDisease: 1, bloodBoilHeartStrikes: 2, boneShield: 12),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "normal Blood Boil is manually consumed at two charges despite available runes and high runic power");
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 70, bloodBoilCharges: 1, targetDisease: 1,
            bloodBoilChargeRemaining: 90, gcdDuration: 100, bloodBoilNearCap: true, boneShield: 12),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "Blood Boil predicts a one-charge refill reaching two charges by the next GCD");
    Equal("心脏打击", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 70, bloodBoilCharges: 1, targetDisease: 1,
            bloodBoilChargeRemaining: 150, gcdDuration: 100, boneShield: 12),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "Blood Boil does not preempt Heart Strike when the next charge is not imminent");
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 70, bloodBoilCharges: 1, targetDisease: 0,
            bloodBoilChargeRemaining: 150, gcdDuration: 100, boneShield: 12),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "Blood Boil repairs missing Blood Plague with a single charge");
    Equal(true, Equals("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 30, bloodBoilCharges: 2, health: 40, targetDisease: 1, boneShield: 12),
        new ContractKeymapResolver()).UnitInfo["动作技能"]),
        "normal Blood Boil remains available at forty percent health when survival rules do not trigger");
    ...
    Equal("血液沸腾", ModuleLogic.Run(
        ...
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "captured combat replay selects normal Blood Boil at two charges when optional cooldown fields are stale");
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 60, bloodBoilCharges: 2, targetDisease: 1,
            bloodBoilHeartStrikes: 2, heartStrikeCooldown: 1, bloodBoilCooldown: 1, boneShield: 12),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "captured combat replay selects normal Blood Boil when runes and cycle counters are stale");
    ...
    Equal(false, Equals("血液沸腾", ModuleLogic.Run(
        ...
        new ContractKeymapResolver()).UnitInfo["动作技能"]),
        "moving pull remains blocked beyond the melee dispatch range");
    Equal("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
        BloodRotationState(runes: 2, runicPower: 30, bloodBoilCharges: 2, boneShield: 12, moving: true, targetRange: 3),
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "moving pull consumes a full Blood Boil charge when the target is in range");
    Equal(false, Equals("血液沸腾", ModuleLogic.Run(
        diagnosticModule,
    ...
    Equal("心脏打击", ModuleLogic.Run(
        diagnosticModule,
        rateLimitedDeathAndDecayState,
        new ContractKeymapResolver(),
        rateLimitedRuleKeys: new HashSet<string>(StringComparer.Ordinal)
        {
    ...
    foreach (var enemyCount in new[] { 1, 2, 3 })
    {
        var burstDecision = ModuleLogic.Run(
            diagnosticModule,
            BurstState(enemyCount, burstEnabled: 1, deathAndDecayCharges: 1),
            new ContractKeymapResolver());
    Equal("符文刃舞", burstDecision.UnitInfo["动作技能"],
        $"burst replay enters the burst layer after Icebound Fortitude at {enemyCount} target(s)");
    var movingBurstState = BurstState(
        enemyCount,
        burstEnabled: 1,
        deathAndDecayCharges: 1,
        moving: true);
    var movingBurstDecision = ModuleLogic.Run(
        diagnosticModule,
        movingBurstState,
        new ContractKeymapResolver());
    Equal(false, Equals("枯萎凋零", movingBurstDecision.UnitInfo["动作技能"]),
        $"moving state blocks Death and Decay at {enemyCount} target(s)");
    var rangedBurstDecision = ModuleLogic.Run(
        diagnosticModule,
        BurstState(enemyCount, burstEnabled: 1, deathAndDecayCharges: 1, targetRange: 8),
        new ContractKeymapResolver());
    Equal(false, Equals("枯萎凋零", rangedBurstDecision.UnitInfo["动作技能"]),
        $"ranged target blocks Death and Decay at {enemyCount} target(s)");
    var burstAfterDeathAndDecay = BurstState(enemyCount, burstEnabled: 1, deathAndDecayCharges: 1);
    ((Dictionary<string, object?>)burstAfterDeathAndDecay.Values["auras"]!)["血色之地"] = 1;
    var markDecision = ModuleLogic.Run(
        diagnosticModule,
        burstAfterDeathAndDecay,
        new ContractKeymapResolver());
    Equal("符文刃舞", markDecision.UnitInfo["动作技能"],
        $"burst replay consumes major burst before Deathbringer's Mark at {enemyCount} target(s)");
    var eliteMarkState = BurstState(
        enemyCount,
        burstEnabled: 0,
        deathAndDecayCharges: 1,
        targetType: 91);
    ...
    Equal("死神印记", ModuleLogic.Run(
        ...
        eliteMarkState,
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        $"Reaper's Mark accepts an elite target (unit type 91) at {enemyCount} target(s)");
    var normalTargetMarkState = BurstState(
        enemyCount,
        burstEnabled: 0,
        deathAndDecayCharges: 1,
        targetType: 1);
    ...
    var outsideNormalMarkState = BurstState(
        enemyCount,
        burstEnabled: 0,
        deathAndDecayCharges: 1,
        targetType: 1,
        inInstance: false);
    ((Dictionary<string, object?>)outsideNormalMarkState.Values["auras"]!)["白骨之盾层数"] = 8;
    ((Dictionary<string, object?>)outsideNormalMarkState.Values["auras"]!)["血色之地"] = 1;
    Equal("死神印记", ModuleLogic.Run(
        diagnosticModule,
        outsideNormalMarkState,
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "Reaper's Mark accepts a normal target outside an instance without the low-Bone-Shield fallback");
    var insideNormalMarkState = BurstState(
        enemyCount,
        burstEnabled: 0,
        deathAndDecayCharges: 1,
        targetType: 1,
        inInstance: true);
    ((Dictionary<string, object?>)insideNormalMarkState.Values["auras"]!)["白骨之盾层数"] = 8;
    ((Dictionary<string, object?>)insideNormalMarkState.Values["auras"]!)["血色之地"] = 1;
    Equal(false, Equals("死神印记", ModuleLogic.Run(
        diagnosticModule,
        insideNormalMarkState,
        new ContractKeymapResolver()).UnitInfo["动作技能"]),
        "Reaper's Mark keeps the high-value target gate for normal targets inside an instance");
    ...
    var dancingDecision = ModuleLogic.Run(
        diagnosticModule,
        burstAfterMark,
        new ContractKeymapResolver());
    Equal("符文刃舞", dancingDecision.UnitInfo["动作技能"],
        $"burst replay enters Dancing Rune Weapon at {enemyCount} target(s)");
    var raiseDeadState = BurstState(
        enemyCount,
        burstEnabled: 1,
        deathAndDecayCharges: 1,
        raiseDeadCooldown: 0);
    ...
    Equal("亡者复生", ModuleLogic.Run(
        ...
        raiseDeadState,
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        $"burst replay sends Raise Dead through its separate slot at {enemyCount} target(s)");
    var lichborneState = BurstState(
        enemyCount,
        burstEnabled: 1,
        deathAndDecayCharges: 1,
        raiseDeadCooldown: 1,
    ...
    Equal("巫妖之躯", ModuleLogic.Run(
        ...
        lichborneState,
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        $"burst replay sends Lichborne through its separate slot at {enemyCount} target(s)");
    }
    var bossBurstDecision = ModuleLogic.Run(
        diagnosticModule,
        BurstState(enemyCount: 0, burstEnabled: 1, deathAndDecayCharges: 1),
        new ContractKeymapResolver());
    Equal("符文刃舞", bossBurstDecision.UnitInfo["动作技能"],
        "major burst priority precedes Death and Decay for a single hostile boss target");
    var maintenanceDeathAndDecayState = BurstState(
        enemyCount: 1,
        burstEnabled: 0,
        deathAndDecayCharges: 1,
        stationaryDuration: 0);
    ((Dictionary<string, object?>)maintenanceDeathAndDecayState.Values["auras"]!)["血色之地"] = 0;
    Equal(false, Equals("枯萎凋零", ModuleLogic.Run(
        diagnosticModule,
        maintenanceDeathAndDecayState,
        new ContractKeymapResolver()).UnitInfo["动作技能"]),
        "Death and Decay waits for one second stationary during the first fifteen seconds");
    var shortCombatDeathAndDecayState = BurstState(
        enemyCount: 1,
        burstEnabled: 0,
        deathAndDecayCharges: 1,
        stationaryDuration: 1);
    ((Dictionary<string, object?>)shortCombatDeathAndDecayState.Values["auras"]!)["血色之地"] = 0;
    Equal("枯萎凋零", ModuleLogic.Run(
        diagnosticModule,
        shortCombatDeathAndDecayState,
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "Death and Decay fires after one second stationary during the first fifteen seconds");
    var longCombatMovingDeathAndDecayState = BurstState(
        enemyCount: 1,
        burstEnabled: 0,
        deathAndDecayCharges: 1,
        moving: true,
    ...
    Equal(false, Equals("枯萎凋零", ModuleLogic.Run(
        ...
        new ContractKeymapResolver()).UnitInfo["动作技能"]),
        "Death and Decay waits until the player has been stationary for one second");
    var burstOffDecision = ModuleLogic.Run(
        diagnosticModule,
        BurstState(1, burstEnabled: 0),
        new ContractKeymapResolver());
    Equal(false, Equals("符文刃舞", burstOffDecision.UnitInfo["动作技能"]),
        "burst replay keeps Dancing Rune Weapon disabled when the burst switch is off");

    var raidBossDefensiveState = BurstState(
        enemyCount: 3,
        burstEnabled: 0,
        runicPower: 0,
        deathAndDecayCharges: 0,
        playerDeathAndDecayAura: 1,
        targetType: 1);
    raidBossDefensiveState.Values["生命值"] = 10;
    raidBossDefensiveState.Values["首领战"] = 1;
    raidBossDefensiveState.Values["难度"] = 14;
    var raidBossDefensiveDecision = ModuleLogic.Run(
        diagnosticModule,
        raidBossDefensiveState,
        new ContractKeymapResolver());
    Equal(false, Equals("吸血鬼之血", raidBossDefensiveDecision.UnitInfo["动作技能"])
        || Equals("冰封之韧", raidBossDefensiveDecision.UnitInfo["动作技能"]),
        "raid boss encounters do not automatically use Vampiric Blood or Icebound Fortitude");

    var raidTrashDefensiveState = BurstState(
        enemyCount: 3,
        burstEnabled: 0,
        runicPower: 0,
        deathAndDecayCharges: 0,
        playerDeathAndDecayAura: 1,
        targetType: 1);
    raidTrashDefensiveState.Values["生命值"] = 10;
    raidTrashDefensiveState.Values["难度"] = 14;
    Equal("吸血鬼之血", ModuleLogic.Run(
        diagnosticModule,
        raidTrashDefensiveState,
        new ContractKeymapResolver()).UnitInfo["动作技能"],
        "raid trash still allows Vampiric Blood outside a boss encounter");

    var dungeonBossDefensiveState = BurstState(
        enemyCount: 3,
        burstEnabled: 0,
        runicPower: 0,
        deathAndDecayCharges: 0,
    ...
    var existingDeathAndDecayState = BurstState(
        ...
        burstEnabled: 0,
        playerDeathAndDecayAura: 1);
    ((Dictionary<string, object?>)existingDeathAndDecayState.Values["spells"]!)["枯萎凋零层数"] = 1;
    var existingDeathAndDecayDecision = ModuleLogic.Run(
        diagnosticModule,
        existingDeathAndDecayState,
        new ContractKeymapResolver());
    Equal(false, Equals("枯萎凋零", existingDeathAndDecayDecision.UnitInfo["动作技能"]),
        "Death and Decay does not immediately recast while Sanguine Ground is active");

    var noActionState = BloodRotationState(
        runes: 0,
        runicPower: 60,
        bloodBoilCharges: 0,
        targetDisease: 1);
    var noActionDecision = ModuleLogic.Run(
        diagnosticModule,
        noActionState,
        new ContractKeymapResolver());
    Equal("暂停", noActionDecision.UnitInfo["动作技能"],
        "resource-starved blood replay reaches the pause fallback");
    Equal(true, noActionDecision.UnitInfo["候选过滤摘要"]?.ToString()?.Contains(
            "心脏打击: 条件不满足（符文 >= 1，实际 0）",
            StringComparison.Ordinal) == true,
        "blood diagnostics include late fallback rules and the actual failing state value");

    var overflowState = BurstState(1, burstEnabled: 0, runicPower: 85, playerDeathAndDecayAura: 1);
    ((Dictionary<string, object?>)overflowState.Values["spells"]!)["死神印记"] = 1;
    var overflowDecision = ModuleLogic.Run(
        diagnosticModule,
        overflowState,
        new ContractKeymapResolver());
    Equal("灵界打击", overflowDecision.UnitInfo["动作技能"],
        "high runic power replay selects Death Strike before Heart Strike can add more power");
    Equal("符文能量", overflowDecision.CooldownConfirmationStateField,
        "Death Strike confirmation uses runic power instead of runes");
    Equal(85, overflowDecision.CooldownConfirmationInitialValue,
        "Death Strike confirmation records the pre-cast runic power");
    Equal(true, overflowDecision.AllowResourceOnlyConfirmation,
        "Death Strike can confirm through runic power when action events are unavailable");

    var deathKnight = ClassBlocksStore.Load(Path.Combine(repositoryRoot, "Fuyutsui", "class", "DeathKnight.lua"));
    var blood = deathKnight.Specs[1];
    Equal(true, blood.PlayerAuras.Any(aura => aura.Name == "血债" && aura.SpellId == 1310372 && aura.MaxApps == 10),
        "blood class block registers Blood Debt stacks");
    Equal(true, blood.CategorizedStates[ClassStateCatalog.CategoryState]
            .Contains("站定时长", StringComparer.Ordinal),
        "blood class block exposes stationary duration for one-second Death and Decay gating");
    foreach (var (specIndex, spec) in deathKnight.Specs)
    {
        Equal(true,
            spec.CategorizedStates[ClassStateCatalog.CategoryState].Contains("玩家动作序号", StringComparer.Ordinal)
                && spec.CategorizedStates[ClassStateCatalog.CategoryState].Contains("玩家动作技能", StringComparer.Ordinal)
                && spec.CategorizedStates[ClassStateCatalog.CategoryState].Contains("玩家动作状态", StringComparer.Ordinal),
            $"death knight spec {specIndex} exposes player action acknowledgement fields");
    }
    foreach (var spell in new[] { "心脏打击", "灵界打击", "精髓分裂", "符文分流", "反魔法护罩", "死亡脚步" })
    {
        Equal(true, blood.Spells.Any(entry => entry.Name == spell),
            $"blood class block registers {spell}");
    }

    var macros = ClassMacrosStore.Load(Path.Combine(repositoryRoot, "Fuyutsui", "core", "classmacros.lua"));
    var deathKnightMacros = macros.Classes["DEATHKNIGHT"];
    foreach (var spell in new[] { "符文分流", "反魔法护罩", "死亡脚步", "治疗石" })
    {
        Equal(true, deathKnightMacros.StaticSpells.Any(entry => entry.Text == spell),
            $"death knight macros expose {spell}");
    }
}
...
static void BundledModuleInstallationContract()
{
    ...
    try
    {
        ...
        var previousVersionTargetDirectory = Path.Combine(fixtureRoot, "previous-version-target");
        Directory.CreateDirectory(previousVersionTargetDirectory);
        var previousVersionPath = Path.Combine(previousVersionTargetDirectory, "holy-paladin.json");
        File.WriteAllText(
            previousVersionPath,
        ...
        Equal(1, previousVersionUpgrade.UpdatedModules.Count,
            "the fixed holy paladin module upgrades the previous bundled version");
        Equal(File.ReadAllText(sourcePath), File.ReadAllText(previousVersionPath),
            "the previous holy paladin version receives the NPC and absorb fixes");
        ...
        Equal(true, BundledModuleInstaller.IsKnownUpgradeableHash(
                "shigure-blood-deathbringer-12-1",
                "26cc54cd5a06de4c77dc6a9322dab34f8220cd0dfc64fd69873f98847582d5f8"),
            "the observed local blood deathbringer 1.2.1.22 module is upgradeable");
        ...
        var bloodSourceDirectory = Path.Combine(fixtureRoot, "blood-source");
        var bloodTargetDirectory = Path.Combine(fixtureRoot, "blood-target");
        Directory.CreateDirectory(bloodSourceDirectory);
        Directory.CreateDirectory(bloodTargetDirectory);
        var bloodSourcePath = Path.Combine(bloodSourceDirectory, "blood-deathbringer.json");
        var bloodTargetPath = Path.Combine(bloodTargetDirectory, "blood-deathbringer.json");
        File.Copy(
            Path.Combine(FindRepositoryRoot(), "BundledModules", "blood-deathbringer-12.1.json"),
            bloodSourcePath);
        ...
        var oldBloodHash = Convert.ToHexString(SHA256.HashData(File.ReadAllBytes(bloodTargetPath))).ToLowerInvariant();
        var oldBlood = ModuleStore.Parse(File.ReadAllBytes(bloodTargetPath));
        var newBlood = ModuleStore.Parse(File.ReadAllBytes(bloodSourcePath));
        Equal(true, BundledModuleInstaller.IsNewerBundledVersion(newBlood, oldBlood),
            "a newer bundled module version is eligible even when its old hash is not whitelisted");
        Equal(false, BundledModuleInstaller.IsNewerBundledVersion(oldBlood, newBlood),
            "an older bundled module version cannot replace a newer installed module");
        var bloodUpgrade = installer.Install(bloodSourceDirectory, bloodTargetDirectory);
        Equal(1, bloodUpgrade.UpdatedModules.Count,
            "a lower-version same-ID Blood DK module is upgraded automatically");
        Equal(currentBloodModule, File.ReadAllText(bloodTargetPath),
            "the lower-version Blood DK module receives the current bundled strategy");
        var bloodBackupPath = Path.Combine(
            fixtureRoot,
            UserDataLayout.MigrationDirectoryName,
            "bundled-module-upgrades",
            $"blood-deathbringer.{oldBloodHash[..12]}.json");
        Equal(true, File.Exists(bloodBackupPath),
            "a lower-version Blood DK module is backed up before automatic upgrade");
        Equal(oldBloodModule, File.ReadAllText(bloodBackupPath),
            "the Blood DK backup preserves the old strategy for rollback");
    }
    finally
    {
...
static int ValidateHolyPaladinModule(string path)
{
    try
    {
        ...
        Equal(UnitSelectorKind.HighestHealingDeficitWithAura,
            module.Units.Single(unit => unit.Name == "最高实际美德目标").Kind,
            "holy paladin module selects actual Virtue-covered healing deficits");
        ...
        Equal(true, shieldRule.Condition.Contains("目标类型 > 0 && 目标类型 < 100 && 目标距离 > 0 && 目标距离 <= 5", StringComparison.Ordinal)
            && shieldRule.MacroCondition == "harm, nodead"
            && !shieldRule.Condition.Contains("目标正面", StringComparison.Ordinal),
            "Shield of the Righteous requires the dedicated five-yard range check and target macro");
        ...
        Equal(true, module.Rules.Where(rule => rule.Unit == ReservedUnit.Target
                && rule.Condition.Contains("目标类型 == 152", StringComparison.Ordinal))
                .All(rule => rule.Condition.Contains("目标距离 > 0 && 目标距离 <= 38", StringComparison.Ordinal)),
            "friendly NPC healing rules keep a safe distance from the forty-yard boundary");
        ...
        Equal(true, BundledModuleInstaller.IsKnownUpgradeableHash(
            ...
            "the deployed 1.2.1.24 blood module with the old highlighted priority upgrades in place");
            ...
            bool forecastAvailable = true)
        {
            ...
            for (var index = 0; index < health.Length; index++)
            {
                ...
                var burstNeed = deficit > 0
                    ? Math.Min(255, deficit + absorb[index] + 10)
                    : absorb[index];
                var sustainNeed = deficit > 0
                    ? Math.Min(255, deficit + absorb[index] + 15)
                    : absorb[index];
            ...
            if (!forecastAvailable)
            {
                ...
                {
                    if (data is Dictionary<string, object?> mutable)
                    {
                        mutable["预期需求"] = 0;
                        mutable["爆发需求"] = 0;
                        mutable["持续需求"] = 0;
        ...
        Equal("美德道标", Action(Evaluate(State([83, 83, 83, 100, 100], virtueCooldown: 0, forecastAvailable: false))),
            "MOD-06 real group deficit opens Virtue when forecast fields are unavailable");
        Equal("美德道标", Action(Evaluate(State(
            [90, 90, 90, 100, 100],
            absorb: [20, 20, 20, 0, 0],
            virtueCooldown: 0))),
            "MOD-06 mixed health deficits and absorbs open Virtue before absorbs clear");
        Equal("美德道标", Action(Evaluate(State(
            [83, 83, 83, 100, 100],
            absorb: [20, 20, 20, 0, 0],
            virtueCooldown: 0,
            forecastAvailable: false))),
            "MOD-06 real high group damage opens Virtue while absorbs remain");
        Equal(false, Action(Evaluate(State(
            [100, 100, 100, 100, 100],
            absorb: [20, 20, 20, 0, 0],
            virtueCooldown: 0,
            forecastAvailable: false))) == "美德道标",
            "MOD-06 absorb-only pressure does not enter the ordinary real-damage Virtue path");
        Equal("美德道标", Action(Evaluate(State(
            [100, 100, 100, 100, 100],
            absorb: [30, 30, 30, 0, 0],
            virtueCooldown: 0))),
            "MOD-06 absorb-only pressure opens Virtue when the burst forecast reaches three targets and ninety total");
        ...
        Equal("暂停", Action(Evaluate(State(
            ...
            judgmentCooldown: 0,
            targetType: 152))),
            "MOD-17 friendly NPC target blocks fallback Holy Light on a player");
        ...
        Equal("圣光闪现", Action(Evaluate(State(
            ...
            combatTime: 100,
            infusion: 5))),
            "MOD-27 expiring in-combat Infusion converts before it disappears");
        ...
        Equal("正义盾击", Action(Evaluate(State(
            ...
            targetDistance: 5,
            judgmentCooldown: 0))),
            "MOD-29 five-yard range check allows Shield of the Righteous");
        Equal("暂停", Action(Evaluate(State(
            [100, 100, 100, 100, 100],
            holyPower: 5,
            targetDistance: 30,
            judgmentCooldown: 0))),
            "MOD-29 out-of-melee range does not dispatch Shield of the Righteous");
        ...
        var absorbWaitingOffensiveState = State(
            [100, 100, 100, 100, 100],
            stage: 3,
            shockCharges: 2,
            judgmentCooldown: 0,
        ...
        Equal("NpcHealing", npcShock.UnitInfo["动作意图"]?.ToString(),
            "MOD-31 current-target healing is classified as NPC healing only for a friendly NPC");
        Equal("荣耀圣令", Action(Evaluate(State(
            [60, 60, 100, 100, 100],
            holyPower: 3,
            targetType: 152,
            targetHealth: 50))),
            "MOD-31 friendly NPC healing remains exclusive while players also need healing");
        Equal("暂停", Action(Evaluate(State(
            [60, 60, 100, 100, 100],
            absorb: [20, 20, 20, 0, 0],
            holyPower: 3,
            ...
            targetType: 152,
            targetHealth: 100))),
            "MOD-31 friendly NPC target never falls back to healing a player");
        ...
        Equal("美德道标", Action(Evaluate(State(
            ...
            stage: 1,
            virtueCooldown: 0))),
            "MOD-35 group pressure can pre-cast Virtue during the absorb resource reserve");
        Equal("美德道标", Action(Evaluate(State(
            [100, 100, 100, 100, 100],
            aoeType: 2,
            stage: 3,
...
static void TopRowBoundaries()
{
    ...
    Equal(5, decoded.Count, "decoded field count");
    Equal(11, decoded[1], "step 1");
    Equal(22, decoded[255], "step 255");
    Equal(33, decoded[256], "step 256");
    Equal(44, decoded[519], "step 519");
    Equal(55, decoded[520], "step 520");
}

static void TopRowRequiresStartMarker()
{
    var decoded = PixelProtocolDecoder.DecodeTopRow(
        new[] { EncodeStep(2, 10), EncodeStep(255, 20), EncodeStep(519, 30), EncodeStep(520, 40) });

    Equal(-1, PixelProtocolDecoder.FindTopRowStart(
        new[] { EncodeStep(2, 10), EncodeStep(255, 20), EncodeStep(519, 30), EncodeStep(520, 40) }), "missing start marker index");
...
static void ModuleMissingBindingFallbackContract()
{
    ...
    Equal(true, decision.UnitInfo["候选过滤摘要"]?.ToString()?.Contains("缺少按键", StringComparison.Ordinal) == true,
        "candidate diagnostics identify why a higher-priority rule was filtered");
...
static void StateBuilderFixture()
{
    ...
    try
    {
        ...
        Equal(17, state.GetInt("目标身份序号"), "fixed target identity field");
        ...
        File.WriteAllText(
            ...
            """
            {
              ...
              "AOE事件类型": { "step": 4, "type": "int" },
              "AOE事件阶段": { "step": 5, "type": "int" },
              "多人爆发需求": { "step": 11, "type": "int" },
              "多人持续需求": { "step": 12, "type": "int" },
              "预计治疗人数": { "step": 13, "type": "int" },
              "5": {
                "2": {
                  "group": {
                    "start": 20,
                    ...
                    "生命值": { "step": 0, "type": "int" },
                    "职责": { "step": 1, "type": "int" },
                    "预期需求": { "step": 2, "type": "int" },
                    "爆发需求": { "step": 3, "type": "int" },
                    "持续需求": { "step": 4, "type": "int" }
                  }
                }
              }
            }
            """);
        var absorbAwareState = new StateBuilder(new ConfigService(fixturePath)).Build(
            new Dictionary<int, int>
            {
                [1] = 233, [2] = 5, [3] = 2, [4] = 0, [5] = 0,
                [11] = 0, [12] = 0, [13] = 0,
                [20] = 100, [21] = 1, [22] = 5, [23] = 0, [24] = 0,
                [25] = 100, [26] = 1, [27] = 5, [28] = 0, [29] = 0
            },
            new Dictionary<int, int>(),
            new Dictionary<int, int> { [1] = 20, [2] = 20 });
        Equal(25, Convert.ToInt32(absorbAwareState.Group["1"]["预期需求"]),
            "decoded healing absorb raises per-unit forecast demand");
        Equal(40, absorbAwareState.GetInt("多人爆发需求"),
            "decoded healing absorb raises the aggregate burst forecast");
        Equal(2, absorbAwareState.GetInt("预计治疗人数"),
            "decoded healing absorb raises the affected-unit count");
        Equal(2, absorbAwareState.GetInt("治疗吸收单位数"),
            "decoded healing absorb publishes the positive-unit count");
        Equal(0, absorbAwareState.GetInt("AOE事件类型"),
            "decoded healing absorb does not fabricate an AOE event");
        Equal(0, absorbAwareState.GetInt("AOE事件阶段"),
            "decoded healing absorb does not bypass the AOE warning stage");
    }
    finally
    {
...
static void ModuleDependencyCaptureAndImportContract()
{
    ...
    try
    {
        ...
        Equal(11, paladinModule.Dependencies.Config.Spec.Group.Num,
        ...
        Equal(11, repairedGroup.Num, "group stride preserves the retained Virtue coverage slot and range flag");
        Equal(7, repairedGroup.ExpectedNeed, "group dependency save preserves single-target threshold");
        Equal(8, repairedGroup.BurstNeed, "group dependency save preserves burst threshold");
        Equal(9, repairedGroup.SustainNeed, "group dependency save preserves sustain threshold");
...
static void CooldownConfirmationTrackerContract()
{
    ...
    Equal(false, queuedSameActionTracker.CanAttempt(
            ...
            allowPreemption: false,
            out _),
        "a healing target change remains blocked while the previous target is still valid");
    Equal(false, queuedSameActionTracker.CanAttempt(
            urgentRetarget,
            GcdState(5),
            now.AddMilliseconds(200),
    ...
    var staleTargetTracker = new CooldownConfirmationTracker();
    staleTargetTracker.RecordSent(decision, now, new GameState(new Dictionary<string, object?>
    {
        ["group"] = new Dictionary<string, IReadOnlyDictionary<string, object?>>
        {
            ["1"] = new Dictionary<string, object?> { ["生命值"] = 100, ["治疗吸收"] = 0 },
            ["2"] = new Dictionary<string, object?> { ["生命值"] = 80, ["治疗吸收"] = 0 }
        }
    }));
    var replacementTarget = decision with
    {
        UnitInfo = new Dictionary<string, object?> { ["动作技能"] = "美德道标", ["动作单位槽位"] = 2 }
    };
    Equal(true, staleTargetTracker.CanAttempt(
            replacementTarget,
            ...
            allowPreemption: false,
            out _),
        "an invalidated healing target can fail over to another target before confirmation");
    staleTargetTracker.RecordSent(
        replacementTarget,
        now.AddMilliseconds(100),
        new GameState(new Dictionary<string, object?>
        {
    ...
    var retargeted = staleTargetTracker.Observe(
        new GameState(new Dictionary<string, object?>
        {
            ["玩家动作序号"] = 1,
            ["玩家动作技能"] = 0,
            ["玩家动作状态"] = 0,
            ["spells"] = new Dictionary<string, object?> { ["美德道标"] = 1 },
            ["group"] = new Dictionary<string, IReadOnlyDictionary<string, object?>>
            {
                ["1"] = new Dictionary<string, object?> { ["生命值"] = 100, ["治疗吸收"] = 0 },
                ["2"] = new Dictionary<string, object?> { ["生命值"] = 80, ["治疗吸收"] = 0 }
        ...
        now.AddMilliseconds(200));
    Equal(true, retargeted.Count == 1 && retargeted[0].Confirmed
        && retargeted[0].Actions.Contains(new LogicActionKey("美德道标", 2))
        && !retargeted[0].Actions.Contains(new LogicActionKey("美德道标", 1)),
        "a failed-over target cannot retain the previous target attribution");
    ...
    Equal(false, tracker.CanAttempt(
            ...
            allowPreemption: false,
            out _),
        "Sacrifice Blessing cannot race a pending confirmation without explicit preemption");
    Equal(true, tracker.CanAttempt(
            emergencyDecision,
            GcdState(80),
            now.AddMilliseconds(10),
            allowPreemption: true,
            out _),
        "a higher-priority emergency action can replace a pending confirmation only with explicit preemption");
    ...
    var unattributedOffGcdFailureTracker = new CooldownConfirmationTracker();
    var unattributedOffGcdDecision = new LogicDecision(
        "ALT-F5",
        "施放 巫妖之躯",
        new Dictionary<string, object?>
        {
        ...
        CooldownConfirmationSpell: "巫妖之躯");
    unattributedOffGcdFailureTracker.RecordSent(
        unattributedOffGcdDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
            ["spells"] = new Dictionary<string, object?> { ["巫妖之躯"] = 0 },
            ["玩家动作序号"] = 114
        }));
    var unattributedOffGcdFailure = unattributedOffGcdFailureTracker.Observe(
        new GameState(new Dictionary<string, object?>
        {
            ["spells"] = new Dictionary<string, object?> { ["巫妖之躯"] = 60 },
            ["玩家动作序号"] = 115,
            ["玩家动作技能"] = 0,
            ["玩家动作状态"] = 4
        }),
        now.AddMilliseconds(100)).Single();
    Equal(false, unattributedOffGcdFailure.Confirmed,
        "an unattributed off-GCD failure cannot be confirmed by a cooldown change");
    Equal(true, unattributedOffGcdFailure.DefinitiveFailure,
        "an unattributed off-GCD failure is retained as a definitive failure");
    ...
    Equal(true, priorityTracker.CanAttempt(
            ...
            allowPreemption: false,
            out _),
        "a higher-value healing action can preempt a pending offense confirmation");
    ...
    Equal(false, gcdPreemptionTracker.CanAttempt(
            ...
            allowPreemption: false,
            out _),
        "a higher-priority GCD heal still waits while the GCD is active");
    Equal(true, gcdPreemptionTracker.CanAttempt(
            healingDecision,
            GcdState(5),
            now.AddMilliseconds(20),
            allowPreemption: false,
            out _),
        "a higher-priority GCD heal remains independent of the GCD remainder");
    ...
    Equal(false, untrackedCooldownlessAction.CanAttempt(
            ...
            allowPreemption: false,
            out _),
        "a cooldown-less ordinary action still waits while the real GCD is active");
    Equal(false, untrackedCooldownlessAction.CanAttempt(
            flashDecision,
            GcdState(50),
            now,
            allowPreemption: false,
            out _),
        "a cooldown-less ordinary action still waits outside the narrow GCD queue window");
    Equal(false, untrackedCooldownlessAction.CanAttempt(
            flashDecision,
            GcdState(51),
            now,
            allowPreemption: false,
            out _),
        "a cooldown-less ordinary action is not independent of the remaining GCD");
    Equal(true, untrackedCooldownlessAction.CanAttempt(
            ...
            allowPreemption: false,
            out _),
        "a cooldown-less ordinary action can be requested inside the GCD queue window");
    ...
    Equal(false, cleanseTracker.CanAttempt(
        ...
        Equal(false, cleanseTracker.Observe(State(0), now.AddMilliseconds(1100)).Single().Confirmed,
    ...
    Equal(false, tracker.Observe(State(0), now.AddMilliseconds(1100)).Single().Confirmed,
    ...
    Equal(true, tracker.CanAttempt(
            ...
            allowPreemption: false,
            out _,
            postConfirmationHold: TimeSpan.Zero),
        "a GCD action can use the real GCD gate without an extra confirmation hold");

    var bloodDecision = new LogicDecision(
        "CTRL-BLOOD",
        "施放 心脏打击",
        new Dictionary<string, object?>
        {
        ...
        CooldownConfirmationSpell: "心脏打击",
        PlayerActionCode: 5);
    var bloodTracker = new CooldownConfirmationTracker();
    bloodTracker.RecordSent(
        bloodDecision,
        now,
        ActionState(0, gcdDuration: 110),
        queueWindowCentiseconds: CooldownConfirmationTracker.BloodDeathKnightQueueWindowCentiseconds);
    ...
    Equal(true, CooldownConfirmationTracker.IsOffGlobalCooldownSpell("亡者复生")
        && CooldownConfirmationTracker.IsOffGlobalCooldownSpell("巫妖之躯"),
        "Blood DK burst companions bypass the GCD gate when sent as separate actions");

    tracker.RecordSent(decision, now);
    var timedOut = tracker.Observe(State(0), now.AddMilliseconds(1100)).Single();
    ...
    static GameState ActionState(
        ...
        int actionCode = 0,
        int actionStatus = 0,
        int gcdDuration = 150) => new(new Dictionary<string, object?>
    {
        ["公共冷却剩余"] = gcdRemaining,
        ["公共冷却时长"] = gcdDuration,
        ["玩家动作序号"] = serial,
    ...
    Equal(false, actionTracker.CanAttempt(
            ...
            allowPreemption: false,
            out _),
        "a first ordinary GCD action waits while the real GCD is active");
    Equal(true, actionTracker.CanAttempt(
            ...
            allowPreemption: false,
            out _),
        "a first GCD action request is independent of the GCD remainder");
    Equal(true, actionTracker.CanAttempt(
            emergencyDecision,
            ActionState(80),
            now,
    ...
    Equal(false, actionTracker.CanAttempt(
            ...
            allowPreemption: false,
            out _),
        "the 300ms retry cadence cannot turn into input flooding");
    ...
    var lateGcdHealingTracker = new CooldownConfirmationTracker();
    lateGcdHealingTracker.RecordSent(castDecision, now, ActionState(0, serial: 10));
    lateGcdHealingTracker.RecordSent(
        decision with { PlayerActionCode = 24 },
        now.AddMilliseconds(10),
        ActionState(0, serial: 10),
        allowTargetReplacement: true);
    Equal(0, lateGcdHealingTracker.Observe(
            ActionState(0, serial: 11, actionCode: 22, actionStatus: 4),
            now.AddMilliseconds(100)).Count,
        "a late failed GCD heal from before Virtue preemption cannot fail Virtue confirmation");
    Equal(true, lateGcdHealingTracker.Observe(
            ActionState(0, serial: 12, actionCode: 24, actionStatus: 2),
            now.AddMilliseconds(200)).Single().Confirmed,
        "Virtue still requires and accepts its own successful action event after preemption");

    var runeDecision = new LogicDecision(
        "CTRL-R",
        "施放 心脏打击",
        new Dictionary<string, object?>
        {
        ...
        CooldownConfirmationSpell: "心脏打击",
        CooldownConfirmationStateField: "符文",
        CooldownConfirmationInitialValue: 2,
        PlayerActionCode: 5,
        AllowResourceOnlyConfirmation: true);
    var runeTracker = new CooldownConfirmationTracker();
    runeTracker.RecordSent(
        runeDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
            ["符文"] = 2,
            ["spells"] = new Dictionary<string, object?> { ["心脏打击"] = 0 },
            ["玩家动作序号"] = 7,
            ["玩家动作技能"] = 0,
            ["玩家动作状态"] = 0
        }));
    var runeConfirmation = runeTracker.Observe(
        new GameState(new Dictionary<string, object?>
        {
            ["符文"] = 1,
            ["spells"] = new Dictionary<string, object?> { ["心脏打击"] = 0 },
        ...
        now.AddMilliseconds(100)).Single();
    Equal(true, runeConfirmation.Confirmed,
        "rune consumption confirms a Blood DK cast without an addon action event");
    Equal("符文", runeConfirmation.StateField,
        "rune-only confirmation records the resource field");
    Equal("状态字段变化：符文", runeConfirmation.ConfirmationSource,
        "rune consumption is reported as the confirmation source");

    var runicPowerDecision = runeDecision with
    {
        CooldownConfirmationSpell = "灵界打击",
        CooldownConfirmationStateField = "符文能量",
        CooldownConfirmationInitialValue = 85
    };
    var runicPowerTracker = new CooldownConfirmationTracker();
    runicPowerTracker.RecordSent(
        runicPowerDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
    ...
    var runicPowerConfirmation = runicPowerTracker.Observe(
        new GameState(new Dictionary<string, object?>
        {
            ["符文能量"] = 40,
            ["spells"] = new Dictionary<string, object?> { ["灵界打击"] = 0 },
        ...
        now.AddMilliseconds(100)).Single();
    Equal(true, runicPowerConfirmation.Confirmed,
        "runic power consumption confirms Death Strike without an addon action event");
    Equal("符文能量", runicPowerConfirmation.StateField,
        "Death Strike confirmation records the runic power field");
    ...
    var interleavedActionTracker = new CooldownConfirmationTracker();
    interleavedActionTracker.RecordSent(
        resourceFailureDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
    ...
    var interleavedConflict = interleavedActionTracker.Observe(
            new GameState(new Dictionary<string, object?>
            {
                ["符文能量"] = 40,
                ["spells"] = new Dictionary<string, object?> { ["灵界打击"] = 1 },
                ["玩家动作序号"] = 21,
                ["玩家动作技能"] = 11,
                ["玩家动作状态"] = 2
            }),
            now.AddMilliseconds(100)).Single();
    Equal(false, interleavedConflict.Confirmed,
        "a different successful action cannot confirm Death Strike from a resource or cooldown change");
    Equal(false, interleavedConflict.DefinitiveFailure,
        "an interleaved action is ambiguous rather than a definitive Death Strike failure");
    Equal(true, interleavedConflict.ConflictingActionObserved,
        "the confirmation records that another action interrupted attribution");
    Equal(false, interleavedActionTracker.CanAttempt(
            runicPowerDecision,
            now.AddMilliseconds(100),
            allowPreemption: false,
            out _),
        "an interleaved action holds the same decision briefly to avoid an immediate duplicate");
    Equal(true, interleavedActionTracker.CanAttempt(
            runicPowerDecision,
            now.AddMilliseconds(301),
            allowPreemption: false,
            out _),
        "an interleaved action becomes retryable on the next short request cadence");

    var queuedResourceConflictTracker = new CooldownConfirmationTracker();
    queuedResourceConflictTracker.RecordSent(
        resourceFailureDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
    ...
    var queuedResourceConflict = queuedResourceConflictTracker.Observe(
        new GameState(new Dictionary<string, object?>
        {
            ["符文能量"] = 40,
            ["spells"] = new Dictionary<string, object?> { ["灵界打击"] = 0 },
        ...
        now.AddMilliseconds(100)).Single();
    Equal(false, queuedResourceConflict.Confirmed,
        "a mismatched queued success cannot confirm a resource-only action");
    Equal(true, queuedResourceConflict.ConflictingActionObserved,
        "a mismatched queued success is retained as an attribution conflict");

    var failedInterleavedActionTracker = new CooldownConfirmationTracker();
    failedInterleavedActionTracker.RecordSent(
        resourceFailureDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
    ...
    var failedInterleavedConflict = failedInterleavedActionTracker.Observe(
            new GameState(new Dictionary<string, object?>
            {
                ["符文能量"] = 40,
                ["spells"] = new Dictionary<string, object?> { ["灵界打击"] = 0 },
            ...
            now.AddMilliseconds(100)).Single();
    Equal(false, failedInterleavedConflict.Confirmed,
        "a different failed action cannot be attributed as a Death Strike failure");
    Equal(false, failedInterleavedConflict.DefinitiveFailure,
        "a different failed action does not create a definitive Death Strike failure");

    var offGcdInterleavedTracker = new CooldownConfirmationTracker();
    var raiseDeadDecision = new LogicDecision(
        "ALT-R",
        "施放 亡者复生",
        new Dictionary<string, object?> { ["动作技能"] = "亡者复生", ["动作单位槽位"] = 0 },
        CooldownConfirmationSpell: "亡者复生");
    offGcdInterleavedTracker.RecordSent(
        raiseDeadDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
            ["spells"] = new Dictionary<string, object?> { ["亡者复生"] = 0 },
            ["玩家动作序号"] = 30
        }));
    var offGcdInterleavedConflict = offGcdInterleavedTracker.Observe(
            new GameState(new Dictionary<string, object?>
            {
                ["spells"] = new Dictionary<string, object?> { ["亡者复生"] = 90 },
                ["玩家动作序号"] = 31,
                ["玩家动作技能"] = 6,
                ["玩家动作状态"] = 4
            }),
            now.AddMilliseconds(100)).Single();
    Equal(false, offGcdInterleavedConflict.Confirmed,
        "a mismatched failed action cannot confirm an off-GCD cooldown change");
    Equal(false, offGcdInterleavedConflict.DefinitiveFailure,
        "a mismatched off-GCD action remains ambiguous rather than a hard failure");
    Equal(true, offGcdInterleavedTracker.CanAttempt(
            raiseDeadDecision,
            now.AddMilliseconds(301),
            allowPreemption: false,
            out _),
        "a mismatched off-GCD confirmation becomes retryable after the short hold");
    ...
    Equal(0, protectedActionTracker.Observe(
            ...
            now.AddMilliseconds(100)).Count,
        "an unattributed protected player cast cannot confirm the pending action");

    var unattributedSuccessTracker = new CooldownConfirmationTracker();
    unattributedSuccessTracker.RecordSent(castDecision, now, ActionState(0));
    var unattributedSuccess = unattributedSuccessTracker.Observe(
        ActionState(0, serial: 1, actionCode: 0, actionStatus: 2),
        now.AddMilliseconds(3_100)).Single();
    Equal(false, unattributedSuccess.Confirmed,
        "an anonymous success cannot confirm a known player action");
    Equal(true, unattributedSuccess.UnattributedActionObserved,
        "an anonymous success is reported as an unattributed observation");

    var failedTracker = new CooldownConfirmationTracker();
    failedTracker.RecordSent(castDecision, now, ActionState(5, gcdDuration: 35));
    var failedAction = failedTracker.Observe(
            ActionState(90, serial: 1, actionCode: 22, actionStatus: 4),
            now.AddMilliseconds(100)).Single();
    Equal(false, failedAction.Confirmed,
        "a definitive failure is reported immediately even while the GCD is active");
    Equal(true, failedAction.DefinitiveFailure,
        "a matching action failure is attributed to the pending spell");

    var failureReasonTracker = new CooldownConfirmationTracker();
    failureReasonTracker.RecordSent(castDecision, now, ActionState(5, gcdDuration: 35));
    var failureWithReason = failureReasonTracker.Observe(
        new GameState(new Dictionary<string, object?>
        {
            ["公共冷却剩余"] = 0,
            ["玩家动作序号"] = 1,
        ...
        now.AddMilliseconds(100)).Single();
    Equal(17, failureWithReason.ObservedFailureReason,
        "the action queue preserves the WoW client failure reason code");
    Equal(false, failedTracker.CanAttempt(
            castDecision,
            ActionState(80, serial: 1, actionCode: 22, actionStatus: 4),
            now.AddMilliseconds(700),
            allowPreemption: false,
            out _),
        "a definitively failed action still waits while WoW reports an active GCD");
    Equal(true, failedTracker.CanAttempt(
            castDecision,
            ActionState(5, serial: 1, actionCode: 22, actionStatus: 4),
            now.AddMilliseconds(700),
            allowPreemption: false,
            out _),
        "a definitively failed action becomes retryable after the GCD queue window");
    ...
    unattributedFailureTracker.RecordSent(
        ...
        now,
        ActionState(0, serial: 10));
    Equal(0, unattributedFailureTracker.Observe(
            ActionState(0, serial: 11, actionCode: 0, actionStatus: 4),
            now.AddMilliseconds(100)).Count,
        "an unattributed failed action cannot release the pending confirmation");

    var queuedActionTracker = new CooldownConfirmationTracker();
    queuedActionTracker.RecordSent(
        resourceFailureDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
    ...
    var queuedActionConfirmation = queuedActionTracker.Observe(
        new GameState(new Dictionary<string, object?>
        {
            ["符文能量"] = 40,
            ["spells"] = new Dictionary<string, object?> { ["灵界打击"] = 0 },
        ...
        now.AddMilliseconds(100)).Single();
    Equal(true, queuedActionConfirmation.Confirmed,
        "the action queue uses the newest matching event instead of a stale interleaved event");
    Equal(8, queuedActionConfirmation.ObservedActionCode,
        "queued confirmation reports the event that was actually consumed");

    var queuedNewestConflictTracker = new CooldownConfirmationTracker();
    queuedNewestConflictTracker.RecordSent(
        resourceFailureDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
    ...
    var queuedNewestConflict = queuedNewestConflictTracker.Observe(
        new GameState(new Dictionary<string, object?>
        {
            ["符文能量"] = 40,
            ["spells"] = new Dictionary<string, object?> { ["灵界打击"] = 1 },
        ...
        now.AddMilliseconds(100)).Single();
    Equal(false, queuedNewestConflict.Confirmed,
        "a newer unrelated queued event cannot overwrite attribution");
    Equal(true, queuedNewestConflict.ConflictingActionObserved,
        "the newer unrelated queued event is reported as a conflict");

    var queuedMatchingBeforeConflictTracker = new CooldownConfirmationTracker();
    queuedMatchingBeforeConflictTracker.RecordSent(
        resourceFailureDecision,
        now,
        new GameState(new Dictionary<string, object?>
        {
    ...
    var matchingBeforeConflict = queuedMatchingBeforeConflictTracker.Observe(
        new GameState(new Dictionary<string, object?>
        {
            ["符文能量"] = 40,
            ["spells"] = new Dictionary<string, object?> { ["灵界打击"] = 1 },
        ...
        now.AddMilliseconds(100)).Single();
    Equal(false, matchingBeforeConflict.Confirmed
        && matchingBeforeConflict.ConflictingActionObserved,
        "a newer unrelated action prevents an earlier matching event from confirming the pending action");
}
...
static void ActionFailureBackoffContract()
{
    ...
    var ambiguousSingle = failed with { DefinitiveFailure = false };
    Equal(false, backoff.Observe(ambiguousSingle, now),
        "the first single-action timeout remains retryable");
    Equal(true, backoff.GetSuppressed(now.AddMilliseconds(100)).Contains(action),
        "the first single-action timeout applies a short retry backoff");
    Equal(true, backoff.Observe(ambiguousSingle, now.AddSeconds(1)),
        "the second single-action timeout activates backoff");
    ...
    var definitiveBackoff = new ActionFailureBackoff();
    Equal(false, definitiveBackoff.Observe(failed, now), "first definite failure remains retryable");
    Equal(true, definitiveBackoff.GetSuppressed(now.AddMilliseconds(100)).Contains(action),
        "a definite failure applies a short retry backoff");
    ...
    definitiveBackoff.Observe(new CooldownConfirmationUpdate(
        "清洁术", false, 0, null, null, null, now,
        new HashSet<LogicActionKey> { firstTarget }), now);
    definitiveBackoff.Observe(new CooldownConfirmationUpdate(
        "清洁术", false, 0, null, null, null, now,
        new HashSet<LogicActionKey> { firstTarget }), now.AddSeconds(1));
    Equal(true, definitiveBackoff.GetSuppressed(now.AddSeconds(1)).Contains(firstTarget),
    ...
    var conflictingAction = new CooldownConfirmationUpdate(
        "灵界打击", false, 0, "符文能量", 90, 90, now,
        new HashSet<LogicActionKey> { deathStrike },
        ExpectedPlayerActionCode: 8,
        ObservedActionCode: 23,
        ObservedActionStatus: 4,
        DefinitiveFailure: false,
        ConflictingActionObserved: true);
    var conflictBackoff = new ActionFailureBackoff();
    Equal(false, conflictBackoff.Observe(conflictingAction, now),
        "a conflicting action event does not count as a Death Strike failure");
    Equal(false, conflictBackoff.GetSuppressed(now.AddSeconds(1)).Contains(deathStrike),
        "a conflicting action event does not suppress the requested spell");
}

static void InterruptFailureDiagnosticContract()
{
    static GameState State(params (string Key, int Value)[] values) =>
        new(values.ToDictionary(value => value.Key, value => (object?)value.Value));

    var target = new HashSet<LogicActionKey> { new("心灵冰冻", ReservedUnit.Target) };
    var focus = new HashSet<LogicActionKey> { new("心灵冰冻", ReservedUnit.Focus) };

    var targetGone = ShigureRuntime.DescribeInterruptState(
        State(("目标类型", 0), ("目标生命值", 0), ("目标施法可打断", 0), ("目标施法(倒计时)", 0)),
        target);
    Equal(true, targetGone.Contains("目标已消失或死亡", StringComparison.Ordinal),
        "interrupt diagnostics identify a dead or replaced target");

    var targetTooEarly = ShigureRuntime.DescribeInterruptState(
        State(("目标类型", 1), ("目标生命值", 80), ("目标距离", 3), ("目标施法可打断", 1), ("目标施法(倒计时)", 12)),
        target);
    Equal(true, targetTooEarly.Contains("离开最后 0.5 秒窗口", StringComparison.Ordinal),
        "interrupt diagnostics identify a cast that is no longer in the final window");

    var focusEnded = ShigureRuntime.DescribeInterruptState(
        State(("焦点施法可打断", 0), ("焦点施法(倒计时)", 0)),
        focus);
    Equal(true, focusEnded.Contains("焦点当前无可打断读条", StringComparison.Ordinal),
        "interrupt diagnostics identify an ended or replaced focus cast");

    var focusRace = ShigureRuntime.DescribeInterruptState(
        State(("焦点施法可打断", 1), ("焦点施法(倒计时)", 3)),
        focus);
...
static void RuntimeDispatchTargetValidationContract()
{
    ...
    Equal(false,
        ...
        "friendly NPC Holy Shock is blocked at the forty-yard boundary");
    Equal(true,
        ShigureRuntime.IsDispatchTargetValid(Decision(152, 38, 0)),
        "friendly NPC Holy Shock remains valid inside the safe range");
    Equal(true,
    ...
    var unavailableGroupState = new GameState(new Dictionary<string, object?>
    {
        ["group"] = new Dictionary<string, IReadOnlyDictionary<string, object?>>
        {
            ["7"] = new Dictionary<string, object?> { ["可治疗"] = 0 }
        }
    });
    Equal(false,
        ShigureRuntime.IsDispatchTargetValid(Decision(1, 5, 0, 7), unavailableGroupState),
        "dynamic group-target healing is blocked when the member is out of range");
    ...
    static GameState TargetState(int targetType, int distance, int targetDeath = 0) =>
        new(new Dictionary<string, object?>
        {
            ["目标类型"] = targetType,
            ["目标距离"] = distance,
    ...
    Equal(true,
        ShigureRuntime.IsDispatchTargetStable(
            BloodDecision("灵界打击"),
            TargetState(1, 2),
            TargetState(1, 3)),
        "blood DK target remains stable across a small distance change");
    Equal(false,
        ShigureRuntime.IsDispatchTargetStable(
            BloodDecision("灵界打击"),
            TargetState(1, 2),
            TargetState(1, 5)),
        "blood DK target transition with a large distance jump is blocked");
    Equal(false,
        ShigureRuntime.IsDispatchTargetStable(
            BloodDecision("灵界打击"),
            TargetState(1, 2),
            TargetState(0, 0)),
        "blood DK target loss is blocked between stable frames");
    Equal(false,
        ShigureRuntime.IsDispatchTargetStable(
            BloodDecision("灵界打击"),
            TargetState(1, 2),
        ...
        "blood DK target replacement is blocked even when geometry looks unchanged");
    Equal(false,
        ShigureRuntime.IsDispatchTargetValid(new LogicDecision(
            "ALT-NUMPADDECIMAL",
            "血DK移动距离校验",
        ...
        "blood DK melee action is blocked while moving at a marginal range");
    Equal(true,
        ShigureRuntime.IsDispatchTargetValid(new LogicDecision(
            "ALT-NUMPAD7",
            "血DK远程技能校验",
        ...
        "blood DK ranged Death's Caress remains valid at thirty yards while moving");
    Equal(false,
        ShigureRuntime.IsDispatchTargetValid(new LogicDecision(
            "ALT-NUMPAD7",
            "血DK远程技能校验",
        ...
        "blood DK ranged Death's Caress is blocked beyond thirty yards");
    Equal(false,
        ShigureRuntime.IsDispatchTargetValid(new LogicDecision(
            "ALT-NUMPAD7",
            "血DK远程技能校验",
        ...
        "blood DK ranged Death's Caress is blocked without a current target");
    Equal(false,
        ShigureRuntime.IsDispatchTargetValid(new LogicDecision(
            "ALT-.",
            "血DK死神印记目标校验",
        ...
        "blood DK Reaper's Mark is blocked without a current target");
...
static void TargetIdentityStabilityGateContract()
{
    static GameState State(int identity, int targetType = 1, int distance = 2, int death = 0) =>
        new(new Dictionary<string, object?>
        {
            ["目标身份序号"] = identity,
            ["目标类型"] = targetType,
    ...
    var gate = new TargetIdentityStabilityGate();
    var now = DateTimeOffset.Parse("2026-01-01T00:00:00Z");
    gate.Observe(State(1), now);
    Equal(false, gate.IsReady(State(1), now.AddMilliseconds(149)),
        "target identity gate blocks the first stable interval");
    Equal(true, gate.IsReady(State(1), now.AddMilliseconds(150)),
        "target identity gate opens after the stable interval");

    gate.Observe(State(1, targetType: 0, distance: 2), now.AddMilliseconds(200));
    Equal(false, gate.IsReady(State(1), now.AddMilliseconds(400)),
        "target disappearance resets the stability interval");
    gate.Observe(State(2), now.AddMilliseconds(401));
    Equal(false, gate.IsReady(State(2), now.AddMilliseconds(550)),
        "target replacement restarts the stability interval");
    Equal(true, gate.IsReady(State(2), now.AddMilliseconds(551)),
        "replaced target becomes eligible only after stabilization");
}
...
static void EmergencyActionGuardContract()
{
    ...
    var raidOtherMember = State(100, (1, 20), (20, 100));
    raidOtherMember.Values["队伍类型"] = 20;
    Equal(false, guard.Observe(Decision("圣疗术", 1), raidOtherMember).Allowed,
        "raid member one starts its own critical confirmation");
    Equal(true, guard.Observe(Decision("圣疗术", 1), raidOtherMember).Allowed,
        "raid member one is not the player when the player occupies slot twenty");
    var raidSelf = State(100, (20, 20));
    raidSelf.Values["队伍类型"] = 20;
    Equal(false, guard.Observe(Decision("圣疗术", 20), raidSelf).Allowed,
        "self-health disagreement protects the actual raid player slot");
    Equal(0, guard.Observe(Decision("圣疗术", 20), raidSelf).ConsecutiveFrames,
        "inconsistent self-health cannot accumulate confirmation frames");
    var playerBeyondEncodedSlots = State(100, (1, 20));
    playerBeyondEncodedSlots.Values["队伍类型"] = 40;
    Equal(40, UnitSelector.ResolvePlayerSlot(playerBeyondEncodedSlots),
        "unencoded raid player slots are not silently aliased to raid member one");
    guard.Reset();
...
static void RuntimeMacroBindingGateContract()
{
    ...
    var runtime = new ShigureRuntime(
        ...
        new MacroBindingUnavailableRuntimeStateBuilder(),
        output,
        new PressedTriggerInput(),
    ...
    runtime.SnapshotUpdated += snapshot =>
    {
        ...
        if (snapshot.CurrentStep.Contains("等待 WoW 宏绑定", StringComparison.Ordinal))
        {
            cancellation.Cancel();
        }
    };
    ...
    Equal(0, output.SendCount, "runtime does not send input before WoW macro bindings are ready");
    Equal(true, snapshots.Any(snapshot =>
            snapshot.CurrentStep.Contains("等待 WoW 宏绑定", StringComparison.Ordinal)
            && snapshot.Enabled),
        "macro binding gate reports a wait state while preserving the user toggle");

    using var deathKnightCancellation = new CancellationTokenSource(TimeSpan.FromMilliseconds(350));
    var deathKnightOutput = new CooldownAwareTargetKeyOutput();
    var deathKnightSnapshots = new System.Collections.Concurrent.ConcurrentQueue<RenderSnapshot>();
    var deathKnightRuntime = new ShigureRuntime(
        new AppOptions(
            "A",
            SendMode.Switch,
            null,
            ...
            TimeSpan.FromMilliseconds(50)),
        new ValidRuntimeScanner(),
        new MacroBindingUnavailableDeathKnightRuntimeStateBuilder(),
        deathKnightOutput,
        new PressedTriggerInput(),
        new CooldownAwareRuntimeLogic(),
        TimeProvider.System);
    deathKnightRuntime.SnapshotUpdated += snapshot =>
    {
        deathKnightSnapshots.Enqueue(snapshot);
        if (snapshot.CurrentStep.Contains("等待 WoW 宏绑定", StringComparison.Ordinal))
        {
            deathKnightCancellation.Cancel();
        }
    };

    try
    {
        deathKnightRuntime.RunAsync(deathKnightCancellation.Token).GetAwaiter().GetResult();
    }
    catch (OperationCanceledException)
    {
    }

    Equal(0, deathKnightOutput.SendCount, "Death Knight runtime does not send input before WoW macro bindings are ready");
    Equal(true, deathKnightSnapshots.Any(snapshot =>
            snapshot.CurrentStep.Contains("等待 WoW 宏绑定", StringComparison.Ordinal)
            && snapshot.Enabled),
        "Death Knight macro binding gate reports a wait state while preserving the user toggle");
}

static void RuntimeProtocolHealthContract()
{
    var now = DateTimeOffset.UtcNow;
    var health = new RuntimeProtocolHealth();

    Equal(false, health.Observe(
            new GameState(new Dictionary<string, object?>()),
            now,
            out var missingReason)
        && missingReason.Contains("字段缺失", StringComparison.Ordinal),
        "runtime protocol health rejects missing handshake fields");

    Equal(false, health.Observe(
            new GameState(new Dictionary<string, object?>
            {
                ["Fuyutsui协议版本"] = RuntimeProtocolHealth.CurrentProtocolVersion + 1,
                ["Fuyutsui状态心跳"] = 1
            }),
            now,
            out var versionReason)
        && versionReason.Contains("协议匹配", StringComparison.Ordinal),
        "runtime protocol health rejects incompatible protocol versions");

    health.Reset();
    var healthy = new GameState(new Dictionary<string, object?>
    {
        ["Fuyutsui协议版本"] = RuntimeProtocolHealth.CurrentProtocolVersion,
        ["Fuyutsui状态心跳"] = 1
    });
    Equal(true, health.Observe(healthy, now, out _), "runtime protocol health accepts the first heartbeat");
    Equal(true, health.Observe(healthy, now.AddMilliseconds(900), out _), "runtime protocol health tolerates a short unchanged interval");
    Equal(false, health.Observe(healthy, now.AddMilliseconds(1101), out var frozenReason)
        && frozenReason.Contains("心跳已冻结", StringComparison.Ordinal),
        "runtime protocol health rejects a frozen heartbeat");
    Equal(true, health.Observe(
            new GameState(new Dictionary<string, object?>
            {
                ["Fuyutsui协议版本"] = RuntimeProtocolHealth.CurrentProtocolVersion,
                ["Fuyutsui状态心跳"] = 2
            }),
            now.AddMilliseconds(1102),
            out _),
        "runtime protocol health recovers when the heartbeat resumes");
}
...
static void HealthstoneUseMacroContract()
{
    ...
    Equal(true, classMacros.Contains("[\"治疗药水\"] = \"/use item:241304\\n/use item:241305\"", StringComparison.Ordinal)
        && classMacros.Contains("[\"浓缩银月城生命药水\"] = \"/use item:271884\\n/use item:271885\"", StringComparison.Ordinal),
        "health potion macros use explicit /use commands and keep item types separate");
    Equal(true, classMacros.Contains("[\"光注法力药水\"] = \"/use item:241300\\n/use item:241301\"", StringComparison.Ordinal)
        && classMacros.Contains("[\"鲁莽药水\"] = \"/use item:241288\\n/use item:241289\"", StringComparison.Ordinal)
        && classMacros.Contains("[\"圣光潜力\"] = \"/use item:241308\\n/use item:241309\"", StringComparison.Ordinal),
        "all item macros use /use instead of the skill /cast command");
...
static void FuyutsuiProtocolContract()
{
    ...
    var spells = File.ReadAllText(Path.Combine(repositoryRoot, "Fuyutsui", "core", "spells.lua"));
    ...
    Equal(true, deathKnight.Contains("[46584]   = { index = 13, name = \"亡者复生\" }", StringComparison.Ordinal)
        && deathKnight.Contains("[46585]   = { index = 13, name = \"亡者复生\" }", StringComparison.Ordinal)
        && deathKnight.Contains("[49039]   = { index = 46, name = \"巫妖之躯\" }", StringComparison.Ordinal)
        && deathKnightConfig.Contains("\"13\": \"亡者复生\"", StringComparison.Ordinal)
        && deathKnightConfig.Contains("\"46\": \"巫妖之躯\"", StringComparison.Ordinal),
        "Blood DK spell event IDs resolve to stable logical action codes");
    Equal(true, deathKnight.Contains("\"副本内\"", StringComparison.Ordinal)
        && player.Contains("function Fuyutsui:UpdateInstanceState()", StringComparison.Ordinal)
        && stateBlocks.Contains("[\"副本内\"] = function() return state.inInstance and 1 / 255 or 0 end", StringComparison.Ordinal)
        && events.Contains("self:UpdateInstanceState()", StringComparison.Ordinal)
        && core.Contains("self:RegisterEvent(\"ZONE_CHANGED_NEW_AREA\")", StringComparison.Ordinal)
        && deathKnightConfig.Contains("\"副本内\"", StringComparison.Ordinal),
        "Blood DK publishes reliable instance state for target-specific Reaper's Mark rules");
    ...
    Equal(true, main.Contains("FALLBACK_MACRO_STATUS_BLOCK = 500", StringComparison.Ordinal)
        && main.Contains("FALLBACK_MACRO_COUNT_BLOCK = 501", StringComparison.Ordinal)
        && main.Contains("FALLBACK_PROTOCOL_VERSION_BLOCK = 502", StringComparison.Ordinal)
        && main.Contains("FALLBACK_PROTOCOL_HEARTBEAT_BLOCK = 503", StringComparison.Ordinal)
        && main.Contains("if not blocks.state[\"宏绑定状态\"] then", StringComparison.Ordinal)
        && main.Contains("if not blocks.state[\"宏绑定数量\"] then", StringComparison.Ordinal)
        && commonConfig.Contains("\"宏绑定状态\"", StringComparison.Ordinal)
        && commonConfig.Contains("\"宏绑定数量\"", StringComparison.Ordinal),
        "macro readiness has a stable fallback block for legacy class resources");
    Equal(true, stateBlocks.Contains("FUYUTSUI_PROTOCOL_VERSION = 6", StringComparison.Ordinal)
        && stateBlocks.Contains("[\"Fuyutsui协议版本\"]", StringComparison.Ordinal)
        && stateBlocks.Contains("[\"Fuyutsui状态心跳\"]", StringComparison.Ordinal)
        && stateBlocks.Contains("[\"目标身份序号\"]", StringComparison.Ordinal)
        && stateBlocks.Contains("失败原因", StringComparison.Ordinal)
        && events.Contains("function Fuyutsui:UpdateProtocolHealth()", StringComparison.Ordinal)
        && events.Contains("self:UpdateProtocolHealth()", StringComparison.Ordinal)
        && commonConfig.Contains("\"Fuyutsui协议版本\"", StringComparison.Ordinal)
        && commonConfig.Contains("\"Fuyutsui状态心跳\"", StringComparison.Ordinal),
        "Fuyutsui publishes a stable protocol version and changing heartbeat");
    ...
    Equal(true,
        player.Contains(
            ...
            StringComparison.Ordinal),
        "unmapped successful addon events do not consume action queue slots");
    Equal(true, player.Contains("local inCombat = UnitAffectingCombat(\"player\")", StringComparison.Ordinal)
        && player.Contains("state.combatStartTime = GetTime()", StringComparison.Ordinal)
        && events.Contains("self:AdvanceTargetIdentity()", StringComparison.Ordinal)
        && target.Contains("function Fuyutsui:AdvanceTargetIdentity()", StringComparison.Ordinal)
        && target.Contains("UnitGUID(unit)", StringComparison.Ordinal)
        && events.Contains("self:UpdatePlayerCombat()", StringComparison.Ordinal)
        && events.Contains("self:UpdateTargetFullInfo()", StringComparison.Ordinal),
        "combat and target state refresh has an OnUpdate fallback when unit events are missed");
    Equal(true,
        target.Contains("local function IsFriendlyNpc(unit)", StringComparison.Ordinal)
        && target.Contains("return isPlayer == false", StringComparison.Ordinal)
        && target.Contains("local guid = UnitGUID(unit)", StringComparison.Ordinal)
        && target.Contains("objectType == \"Creature\"", StringComparison.Ordinal)
        && target.Contains("objectType == \"Vehicle\"", StringComparison.Ordinal)
        && target.Contains("objectType == \"Pet\"", StringComparison.Ordinal),
        "friendly NPC classification falls back to safe GUID types when UnitIsPlayer is secret");
    ...
    Equal(true, block.Contains("local BLOCK_FIX_COUNT = 520", StringComparison.Ordinal)
        && main.Contains("\"失败原因\"", StringComparison.Ordinal)
        && main.Contains("TARGET_IDENTITY_BLOCK = 520", StringComparison.Ordinal)
        && player.Contains("failureReason = state.lastUiErrorType or 0", StringComparison.Ordinal)
        && events.Contains("state.lastUiErrorType", StringComparison.Ordinal)
        ...
        && paladinConfig.Contains("\"num\": 11", StringComparison.Ordinal)
        && paladinConfig.Contains("\"可治疗\"", StringComparison.Ordinal)
    ...
    Equal(true,
        aoeWarning.Contains("significantAbsorbUnits >= 3", StringComparison.Ordinal)
        && aoeWarning.Contains("totalAbsorbPercent >= 45", StringComparison.Ordinal)
        && aoeWarning.Contains("absorbPercent >= 10", StringComparison.Ordinal)
        && aoeWarning.Contains("anyPositive and significantGroupAbsorb", StringComparison.Ordinal)
        && aoeWarning.Contains("warning.observedAbsorbStreak >= 2", StringComparison.Ordinal),
        "reactive absorb fallback requires significant multi-target pressure in two consecutive frames");
    Equal(true, target.Contains("UnitIsPlayer(unit)", StringComparison.Ordinal)
        && target.Contains("return 152 / 255", StringComparison.Ordinal)
        && target.Contains("if canAssist and IsFriendlyNpc(unit) then", StringComparison.Ordinal)
        ...
        && target.Contains("cache.canAssist = UnitCanAssist(\"player\", unit)", StringComparison.Ordinal)
        && !target.Contains("cache.canAssist = false", StringComparison.Ordinal)
        && !target.Contains("cache.type = (cache.canAttack and 92 or 2) / 255", StringComparison.Ordinal)
    ...
    Equal(true, classMacros.Contains("[@target,harm,nodead]正义盾击", StringComparison.Ordinal)
        ...
        && classMacros.Contains("[\"治疗药水\"] = \"/use item:241304\\n/use item:241305\"", StringComparison.Ordinal)
        && classMacros.Contains("[\"美德道标\"] = true", StringComparison.Ordinal)
        && !classMacros.Contains("[\"神圣震击\"] = true", StringComparison.Ordinal)
    ...
    Equal(true, paladin.Contains("\"AOE事件类型\"", StringComparison.Ordinal)
        ...
        && target.Contains("return 1 / 255", StringComparison.Ordinal)
    ...
    Equal(true, deathKnight.Contains("\"公共冷却时长\"", StringComparison.Ordinal)
        ...
        && deathKnight.Contains("\"血沸充能剩余\"", StringComparison.Ordinal)
        && deathKnight.Contains("\"血沸即将满层\"", StringComparison.Ordinal)
        && deathKnight.Contains("\"浓缩银月城生命药水\"", StringComparison.Ordinal)
        && deathKnight.Contains("\"血沸手动触发\"", StringComparison.Ordinal)
        && deathKnight.Contains("\"血沸高亮\"", StringComparison.Ordinal)
        && deathKnight.Contains("\"符文刃舞爆发窗口\"", StringComparison.Ordinal),
        "Blood DK protocol exposes measured GCD and Blood Boil charge timing state");
    Equal(true, stateBlocks.Contains("[\"血沸充能剩余\"]", StringComparison.Ordinal)
        && stateBlocks.Contains("[\"血沸即将满层\"]", StringComparison.Ordinal)
        && spells.Contains("function Fuyutsui:UpdateBloodBoilChargeRemaining()", StringComparison.Ordinal)
        && spells.Contains("GetSpellCharges(50842)", StringComparison.Ordinal)
        && spells.Contains("duration / modRate", StringComparison.Ordinal)
        && spells.Contains("GetEstimatedGCDSeconds", StringComparison.Ordinal)
        && spells.Contains("bloodBoilNearCap", StringComparison.Ordinal)
        && spells.Contains("self:UpdateStateBlock(\"状态\", \"血沸充能剩余\")", StringComparison.Ordinal)
        && spells.Contains("self:UpdateStateBlock(\"状态\", \"血沸即将满层\")", StringComparison.Ordinal)
        && events.Contains("self:UpdateSpellCooldown()", StringComparison.Ordinal)
        && deathKnightConfig.Contains("\"血沸充能剩余\"", StringComparison.Ordinal)
        && deathKnightConfig.Contains("\"血沸即将满层\"", StringComparison.Ordinal),
        "Blood Boil exposes charge recharge timing through the shared state protocol");
    Equal(true, stateBlocks.Contains("[\"浓缩银月城生命药水\"] = function(self) return GetAnyItemAvailabilityPixel(self, \"ConcentratedHealthPotionCount\", { 271884, 271885 }) end", StringComparison.Ordinal)
        && stateBlocks.Contains("[\"治疗药水\"] = function(self) return GetAnyItemAvailabilityPixel(self, \"SilvermoonHealthPotionCount\", { 241304, 241305 }) end", StringComparison.Ordinal)
        && stateBlocks.Contains("C_Item.GetItemCount(itemID) > 0", StringComparison.Ordinal),
        "Blood DK item protocol separates ordinary and concentrated potion availability");
    Equal(true, stateBlocks.Contains("[\"血沸手动触发\"]", StringComparison.Ordinal)
        && player.Contains("state.bloodBoilManualTriggerPending", StringComparison.Ordinal)
        && player.Contains("StartBloodBoilEchoWindow(now)", StringComparison.Ordinal),
        "Blood Boil protocol separates the one-shot manual trigger from the automatic chain");
    ...
    Equal(true, aoeWarning.Contains("function Fuyutsui:GetEstimatedGCDSeconds()", StringComparison.Ordinal)
        ...
        && deathKnightConfig.Contains("\"公共冷却时长\"", StringComparison.Ordinal),
...
static void PeriodicRefreshProductionLuaReplayContract()
{
    var repositoryRoot = FindRepositoryRoot();
    var startInfo = new ProcessStartInfo("/usr/bin/env")
    {
    ...
    startInfo.ArgumentList.Add("luajit");
    startInfo.ArgumentList.Add(Path.Combine(repositoryRoot, "Tests", "Shigure.Core.ContractTests",
        "Fixtures", "periodic-refresh-replay.lua"));
    startInfo.ArgumentList.Add(repositoryRoot);
    using var replay = Process.Start(startInfo)
        ?? throw new InvalidOperationException("failed to start periodic refresh Lua replay");
    var stdout = replay.StandardOutput.ReadToEnd();
    var stderr = replay.StandardError.ReadToEnd();
    replay.WaitForExit();
    if (replay.ExitCode != 0)
    {
        throw new InvalidOperationException($"periodic refresh Lua replay failed:{Environment.NewLine}{stdout}{stderr}");
    }
    Equal(true, stdout.Contains("Periodic refresh production Lua replay passed", StringComparison.Ordinal),
        "production periodic refresh handles restricted charges and publishes heartbeat only after success");
}
...
static void MacUiStatePersistenceContract()
{
    ...
    Equal(100, missing.State.LogicIntervalMilliseconds, "missing Mac UI state uses default logic interval");
    Equal(250, missing.State.RenderIntervalMilliseconds, "missing Mac UI state uses default render interval");
    Equal(null, missing.State.LocalModuleSourceDirectory, "missing Mac UI state uses packaged module source");
        ...
        var state = new MacUiState
        {
            ...
            LogicIntervalMilliseconds = 75,
            RenderIntervalMilliseconds = 400,
        ...
        Equal(75, loaded.State.LogicIntervalMilliseconds, "logic interval round trip");
        Equal(400, loaded.State.RenderIntervalMilliseconds, "render interval round trip");
...
static void RuntimeResourceWorkspaceContract()
{
    ...
    Equal(true, RuntimeResourceWorkspaceService.IsKnownUpgradeableResource(
        ...
        "stale runtime Blood DK class resource is eligible for managed repair");
    Equal(true, RuntimeResourceWorkspaceService.IsKnownUpgradeableResource(
            "Fuyutsui/class/DeathKnight.lua",
            "d6497131835224cd02cdb5152a4c7f7adb0fcb801830bf09455d03369a9ca171"),
        "stale Blood DK class resource missing blood boil trigger state is eligible for managed repair");
    Equal(false, RuntimeResourceWorkspaceService.IsKnownUpgradeableResource(
            "Fuyutsui/class/DeathKnight.lua",
            new string('0', 64)),
        "unknown Blood DK class customization remains protected as a conflict");
    ...
    try
    {
        ...
        var sourcePaladinClass = Path.Combine(sourceRoot, "Fuyutsui", "class", "Paladin.lua");
        ...
        Equal(3, upgraded.ConflictingFiles.Count, "user-managed Fuyutsui resources remain preserved conflicts");
        Equal(true, upgraded.BackedUpFiles.Contains("Fuyutsui/core/state.lua"), "overwritten engine edit is backed up");
        Equal(true, upgraded.BackedUpFiles.Contains("Fuyutsui/core/macro.lua"), "overwritten macro engine edit is backed up");
        Equal(false, upgraded.BackedUpFiles.Contains("Fuyutsui/core/classmacros.lua"), "macro customization is not overwritten");
        Equal(false, upgraded.BackedUpFiles.Contains("Fuyutsui/class/Mage.lua"), "class customization is not overwritten");
        Equal(false, upgraded.BackedUpFiles.Contains("Fuyutsui/class/Paladin.lua"), "Paladin customization is not overwritten");
        var conflictBackupRoot = Path.Combine(
            userDataRoot,
            UserDataLayout.MigrationDirectoryName,
            "runtime-conflicts");
        Equal(2, Directory.EnumerateFiles(conflictBackupRoot, "*", SearchOption.AllDirectories).Count(),
            "only overwritten engine resources have a recovery backup");
        var stateBackup = Directory.EnumerateFiles(conflictBackupRoot, "state.lua", SearchOption.AllDirectories).Single();
        Equal("user-change", File.ReadAllText(stateBackup), "recovery backup preserves the overwritten source");
        ...
        Equal(0, upgraded.ProtocolConflictingFiles.Count, "user-managed Fuyutsui resources do not create protocol conflicts");
        ...
        Equal(false, upgraded.ProtocolConflictingFiles.Contains("Fuyutsui/class/Mage.lua"), "class customization does not block runtime");
        Equal(true, upgraded.MigratedFiles.Contains("Fuyutsui/class/Mage.lua"), "user class customization still receives safe state migration");
        Equal(true, upgraded.MigratedFiles.Contains("Fuyutsui/class/Paladin.lua"), "user Paladin customization still receives safe state migration");
        Equal(true, upgraded.MigratedFiles.Contains("Fuyutsui/core/classmacros.lua"), "user macro customization receives the targeted routing migration");
        ...
        Equal("清洁术", reconciledHoly["route-3-2"]?["技能"]?.GetValue<string>(),
            "reconciled keymap preserves the user macro spell order");
        ...
        Equal("source-v2", File.ReadAllText(targetLua), "packaged engine source is deployed");
        Equal("macro-v2", File.ReadAllText(targetMacro), "packaged macro engine source is deployed");
        Equal(true, File.ReadAllText(targetClassMacros).Contains("common = { \"荣耀圣令\", \"清毒术\" },", StringComparison.Ordinal),
            "user macro additions are preserved");
        Equal(true, File.ReadAllText(targetClassMacros).Contains("[@target,harm,nodead]正义盾击", StringComparison.Ordinal),
            "targeted macro migration still repairs the hostile Shield of the Righteous route");
        Equal(false, File.ReadAllText(targetClass).Contains("\"施法\",", StringComparison.Ordinal),
            "user class customization receives legacy cast-state normalization");
...
static void MacUiTechnicalSampleContract()
{
    ...
    var runtimeText = File.ReadAllText(Path.Combine(FindRepositoryRoot(), "Runtime", "ShigureRuntime.cs"));
    ...
    Equal(true, sourceText.Contains("CommandButton(\"复制\"", StringComparison.Ordinal)
        && sourceText.Contains("CloneSelected", StringComparison.Ordinal),
        "Mac module editor can duplicate selected rows");
    ...
    Equal(true, mainWindowText.Contains(
            ...
            StringComparison.Ordinal),
        "Mac UI builds runtime options from persisted intervals");
    Equal(true, mainWindowText.Contains("IntervalBox(_logicIntervalMilliseconds, 50, 1000)", StringComparison.Ordinal)
        && mainWindowText.Contains("IntervalBox(_renderIntervalMilliseconds, 100, 5000)", StringComparison.Ordinal),
        "Mac UI exposes bounded runtime interval settings");
    ...
    Equal(true, mainWindowText.Contains("snapshot.State?.GetInt(\"有效性\") == 1", StringComparison.Ordinal),
        "Mac UI only reports scan recovery after a valid state is restored");
    Equal(true, runtimeText.Contains("3 or 4 or 6 => null", StringComparison.Ordinal),
        "runtime suppresses mount and chat invalid-state prompts");
    Equal(false, runtimeText.Contains("角色处于坐骑状态", StringComparison.Ordinal)
        || runtimeText.Contains("聊天输入框已打开", StringComparison.Ordinal)
        || runtimeText.Contains("角色正在施放坐骑", StringComparison.Ordinal),
        "runtime does not expose mount or chat prompt text");
...
static void MacPackagingReleaseContract()
{
    ...
    var buildScriptText = File.ReadAllText(Path.Combine(packagingRoot, "build-app.sh"));
    var localRepackageScriptText = File.ReadAllText(Path.Combine(packagingRoot, "repackage-local.sh"));
    ...
    Equal(false, localRepackageScriptText.Contains("--skip-contract-surface-manifest", StringComparison.Ordinal),
        "local full packaging runs the contract surface manifest check");
...
static void FuyutsuiMacroCombatRetryContract()
{
    ...
    var classMacrosText = File.ReadAllText(Path.Combine(repositoryRoot, "Fuyutsui", "core", "classmacros.lua"));
    ...
    var coreText = File.ReadAllText(Path.Combine(repositoryRoot, "Fuyutsui", "core", "core.lua"));
    var playerText = File.ReadAllText(Path.Combine(repositoryRoot, "Fuyutsui", "core", "player.lua"));
    var targetText = File.ReadAllText(Path.Combine(repositoryRoot, "Fuyutsui", "core", "target.lua"));
    ...
    Equal(true, macroText.Contains("local hadReadyBindings = Fuyutsui.state.macroBindingStatus == 1", StringComparison.Ordinal)
        && macroText.Contains("(Fuyutsui.state.macroBindingCount or 0) > 0", StringComparison.Ordinal)
        && macroText.Contains("self.macrosPending = true", StringComparison.Ordinal)
        && mainText.Contains("local hadReadyBindings = self.state.macroBindingStatus == 1", StringComparison.Ordinal)
        && mainText.Contains("(self.state.macroBindingCount or 0) > 0", StringComparison.Ordinal),
        "combat lockdown preserves usable macro bindings and schedules a rebuild");
    Equal(true, macroText.Contains("local function AddCastPreemption", StringComparison.Ordinal)
        && macroText.Contains("BaseSpellName(spell)", StringComparison.Ordinal)
        && macroText.Contains("/stopcasting\\n", StringComparison.Ordinal),
        "only explicitly registered preempting macro spells can stop the current cast");
    Equal(true, macroText.Contains("if spell == \"审判\" then", StringComparison.Ordinal)
        && macroText.Contains("/cast [harm,nodead]%s\\n/cast %s%s", StringComparison.Ordinal)
        && classMacrosText.Contains("[@tanktarget]审判", StringComparison.Ordinal),
        "Judgment uses the current target first and the tank target as a macro fallback");
    var preemptionMarker = "Fuyutsui.CastPreemptingSpells = {";
    var preemptionStart = classMacrosText.IndexOf(preemptionMarker, StringComparison.Ordinal);
    var preemptionEnd = preemptionStart < 0
        ? -1
        : classMacrosText.IndexOf("\n}", preemptionStart, StringComparison.Ordinal);
    var luaPreemptingSpells = preemptionStart >= 0 && preemptionEnd > preemptionStart
        ? classMacrosText[preemptionStart..preemptionEnd]
            .Split('\n', StringSplitOptions.RemoveEmptyEntries)
            .Where(line => line.Contains("] = true", StringComparison.Ordinal))
            .Select(line => line.Split("[\"", StringSplitOptions.None)[1].Split("\"]", StringSplitOptions.None)[0])
            .ToHashSet(StringComparer.Ordinal)
        : new HashSet<string>(StringComparer.Ordinal);
    Equal(true, luaPreemptingSpells.SetEquals(CastPreemptionPolicy.AllowedSpells),
        "Lua and C# cast-preemption policies remain in parity");
    ...
    Equal(true, eventsText.Contains("if self.macrosPending then", StringComparison.Ordinal)
        && eventsText.Contains("C_Timer.After(0, function()", StringComparison.Ordinal)
        && eventsText.Contains("if self.macrosPending and not InCombatLockdown() then", StringComparison.Ordinal)
        && eventsText.Contains("self:LoadPlayerMacros()", StringComparison.Ordinal),
        "leaving combat schedules a safe pending macro retry");
    var initialSpecPosition = coreText.IndexOf("self:GetCharacterSpecInfo()", StringComparison.Ordinal);
    var initialMacroPosition = coreText.IndexOf("self:LoadPlayerMacros()", StringComparison.Ordinal);
    var initialSpellPosition = coreText.IndexOf("self:UpdateSpellKnown()", StringComparison.Ordinal);
    Equal(true, initialSpecPosition >= 0
        && initialMacroPosition > initialSpecPosition
        && initialMacroPosition < initialSpellPosition,
        "addon initialization creates macro bindings immediately after resolving the active specialization");
    Equal(true, mainText.Contains("function Fuyutsui:MacroTrace", StringComparison.Ordinal)
        && !mainText.Contains("print(\"[Fuyutsui宏诊断]", StringComparison.Ordinal)
        && targetText.Contains("目标诊断：exists=%s canAttack=%s", StringComparison.Ordinal)
        && coreText.Contains("OnEnable 开始", StringComparison.Ordinal)
        && playerText.Contains("登录延迟宏绑定回调触发", StringComparison.Ordinal)
        && eventsText.Contains("PLAYER_ENTERING_WORLD 延迟回调触发", StringComparison.Ordinal)
        && eventsText.Contains("PLAYER_REGEN_DISABLED：进入战斗", StringComparison.Ordinal),
        "macro lifecycle diagnostics remain callable without printing test messages to chat");
    Equal(false, classMacrosText.Contains(
            "[\"符文刃舞\"] = \"/cast 亡者复生\\n/cast 巫妖之躯\\n/cast 符文刃舞\"",
            StringComparison.Ordinal),
        "Blood DK no longer relies on a composite burst macro");
    Equal(true, classMacrosText.Contains("\n            \"亡者复生\",", StringComparison.Ordinal)
        && classMacrosText.Contains("\n            \"符文刃舞\",", StringComparison.Ordinal)
        && classMacrosText.Contains("\n            \"巫妖之躯\",", StringComparison.Ordinal),
        "Blood DK keeps separate keymap entries for all three burst actions");
}
...
{
    var zeroBased = step - 1;
    var red = zeroBased / 255;
    var green = zeroBased % 255 + 1;
...
sealed class MacroBindingUnavailableRuntimeStateBuilder : IRuntimeStateBuilder
{
    public GameState Build(
        IReadOnlyDictionary<int, int> rowData,
        IReadOnlyDictionary<int, int> barData,
...
sealed class MacroBindingUnavailableDeathKnightRuntimeStateBuilder : IRuntimeStateBuilder
{
    public GameState Build(
        IReadOnlyDictionary<int, int> rowData,
        IReadOnlyDictionary<int, int> barData,
