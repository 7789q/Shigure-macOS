...
function Fuyutsui:ToggleBurst()
    local c = self:GetCharConfig()
    if not c then return end
    if (self.BurstTime or 0) > GetTime() then
        SetBurstTime(c, -1, 0, "|cff00ff00[Fuyutsui]|r 爆发已|cffff0000关闭|r")
    else
        SetBurstTime(c, math.huge, 1, "|cff00ff00[Fuyutsui]|r 爆发已|cff00ff00开启|r")
    end
end
