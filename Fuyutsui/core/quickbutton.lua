...
摘要：
    ...
    左键 +15 秒 / 右键取消 / 中键切换爆发）+ 双按钮行（自动/单体、不喝药/爆发药）。
描述：
    ...
    残段）；点击静默、按按键分派：左键 +15 秒 / 右键取消（now-1）/ 中键切换爆发 /
...
主要变量信息：
    ...
    c：GetCharConfig() 返回的角色配置表（aoeMode/potion/cooldowns/quickButton*）
...
local PANEL_SCALE = GetDefaultScale() -- 固定到 WoW 默认缩放，不跟随用户 UI Scale
...
local function SaveQuickButtonPosition(self)
    ...
    if p and x and y then
        ...
        c.quickButtonX = x
        c.quickButtonY = y
        c.quickButtonScale = PANEL_SCALE
    end
end

local function ClampQuickButtonPosition(frame, c, point)
    local left, bottom, width, height = frame:GetRect()
    local parentLeft, parentBottom, parentWidth, parentHeight = UIParent:GetRect()
    if not left or not bottom or not parentLeft or not parentBottom then return end

    local parentScale = UIParent:GetEffectiveScale()
    local scaledLeft = left * PANEL_SCALE
    local scaledBottom = bottom * PANEL_SCALE
    local scaledRight = scaledLeft + width * PANEL_SCALE
    local scaledTop = scaledBottom + height * PANEL_SCALE
    local scaledParentLeft = parentLeft * parentScale
    local scaledParentBottom = parentBottom * parentScale
    local scaledParentRight = scaledParentLeft + parentWidth * parentScale
    local scaledParentTop = scaledParentBottom + parentHeight * parentScale
    local deltaX, deltaY = 0, 0

    if scaledLeft < scaledParentLeft then
        deltaX = scaledParentLeft - scaledLeft
    elseif scaledRight > scaledParentRight then
        deltaX = scaledParentRight - scaledRight
    end
    if scaledBottom < scaledParentBottom then
        deltaY = scaledParentBottom - scaledBottom
    elseif scaledTop > scaledParentTop then
        deltaY = scaledParentTop - scaledTop
    end
    if deltaX == 0 and deltaY == 0 then return end

    c.quickButtonX = c.quickButtonX + deltaX / PANEL_SCALE
    c.quickButtonY = c.quickButtonY + deltaY / PANEL_SCALE
    frame:ClearAllPoints()
    frame:SetPoint(point, UIParent, c.quickButtonRelPoint or point, c.quickButtonX, c.quickButtonY)
end
...
function Fuyutsui:InitQuickToggleButton()
    ...
    -- 忽略用户 UI Scale，并固定到 WoW 默认缩放基准，避免面板在缩放变化后变宽或变窄。
    f:SetIgnoreParentScale(true)
    f:SetScale(PANEL_SCALE)
    ...
    if p and c.quickButtonX and c.quickButtonY then
        -- 旧存档未记录 scale，按原 scale=1 迁移；换算后保持屏幕上的实际位置不变。
        local savedScale = c.quickButtonScale or 1
        local positionScale = savedScale / PANEL_SCALE
        c.quickButtonX = c.quickButtonX * positionScale
        c.quickButtonY = c.quickButtonY * positionScale
        c.quickButtonScale = PANEL_SCALE
        f:SetPoint(p, UIParent, c.quickButtonRelPoint or p, c.quickButtonX, c.quickButtonY)
        ClampQuickButtonPosition(f, c, p)
    else
    ...
    local function ShowBurstTooltip()
        ...
        GameTooltip:AddLine("中键：开关爆发", 0.8, 0.8, 0.8, true)
        GameTooltip:Show()
    end
    ...
    -- 中键切换爆发 / 其余按键维持 +15，写 Fuyutsui.BurstTime 并同步写 c.cooldowns（静默）；
    ...
    barLayer:SetScript("OnMouseUp", function(self, button)
        ...
        if math.abs(x - pressX) < CLICK_THRESHOLD and math.abs(y - pressY) < CLICK_THRESHOLD then
            ...
            if cc and button ~= "MiddleButton" then
